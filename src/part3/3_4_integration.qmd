...
$A$ 가 조르당 가측이고 $\varepsilon >0$ 이면 어떤 조르당 가측이며 $A$ 의 부분집합인 옹골집합 $C$ 가 존재하여 $\int_{A-C} 1 < \varepsilon$ 임을 보여라.
:::
...
$f:[a,\,b]\times [c,\,d] \to \R$ 이 연속이며 $D_2 f$ 역시 연속이라고 하자. $\tilde {f} (y) =\int_a^b f(x,\,t)\,dx$ 라고하자. 아래의 식 (Leibnitz's rule) 이 성립함을 보여라.
