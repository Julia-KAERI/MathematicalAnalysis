...
선형변환 $L:\R^n \to \R^m$ 은 정해진 기저에 대해 $m \times n$ 행렬로 표현할 수 있다는 것을 안다. $\R^n$ 의 기저 $\mathcal{B}_n=(\bf{v}_1,\ldots,\,\bf{v}_n)$ 과 $\R^m$ 의 기저 $\mathcal{B}_m=(\bf{u}_1,\ldots,\,\bf{u}_m)$ 에 대한 $L$ 에 대한 행렬 표현을 $[L]_{\mathcal{B}_n,\, \mathcal{B}_m}$ 라고 하자. $\R^m$ 과 $\R^n$ 각각의 표준기저에 대한 행렬표현은 $[L]$ 라고 쓰기로 하자. 즉 $L(\bf{x}) = [L]\cdot \bf{x}$ 이다. 

혼동의 여지가 있지만 $[L]$ 의 $i$ 행 $j$ 열 성분을 $[L]_{ij}$ 라고 표현하기로 하자. 첨자가 인덱스이면 행렬의 성분이며 첨자가 기저이면 특정 기저의 행렬표현이다. 하지만 여기서는 대부분 전자의 의미로 사용되며, 혼동의 여지가 있다면 특정하기로 하자. 즉 $\bf{x} = \begin{bmatrix} x_1 & \ldots & x_n\end{bmatrix}^T \in \R^m$ 과 $\bf{y} = L(\bf{x})$ 에 대해  

$$
y_i = (L(\bf{x}))_i = \sum_{i=1}^m [L]_{ij}x_j
$$
...
함수 $F:\R^n\to \R^m$ 와 $\bf{a}\in \R^n$ 에 대해 어떤 선형변환 $L:\R^n \to \R^m$ 이 존재하여

$$
\lim_{\bf{h} \to \bf{0}} \dfrac{\|F(\bf{a}+\bf{h}) - F(\bf{a}) - L(\bf{h})\|}{\|\bf{h}\|} = 0
$$ {#eq-CM_derivative_of_vector_function}

일 때 $F$ 는 $\bf{a}$ 에서 미분가능하다 라고 하며 $L$ 를 $DF(\bf{a})$ 혹은 $F'(\bf{a})$ 라고 쓴다. 
...
$F:A \subset \R^n \to \R^m$ 에 대해 $F'(\bf{a})$ 를 $\R^n$ 과 $\R^m$ 의 표준기저에 대해 표현한 행렬을 **야코비 행렬 (Jacobian matrix)** 이라고 하며 $J[F](\bf{a})$ 혹은 $J_F(\bf{a})$ 와 같이 표기한다. 모든 $A\subset \R^n$ 에서 미분가능할 때는 $A$ 에서 정의된 함수로서 $J_F(\bf{x})$ 라고 표기한다. 
...
이므로 $\bf{\pi}'(\bf{x})=I_n$ 이다. 또한

$$
\|\pi_i (\bf{x}+\bf{h})-\pi_i (\bf{x})-\pi_i(\bf{h})\| = 0
$$

이므로 $\pi'_i(\bf{x})= \pi_i = \bf{e}_i^T$ 이다. 
...
$$
\dfrac{|f(\bf{h})|}{\|\bf{h}\|}\le \|\bf{h}\|
$$

이므로 $a=b=0$ 에 대해 @eq-CM_spivak_2_7 를 만족한다. 따라서 $f'(\bf{0})= \begin{bmatrix}0 & 0\end{bmatrix}^T$ 이다.
...
$\bf{b}=F(\bf{a})$, $L=F'(\bf{a}),\, T = (G\circ F)'(\bf{a})$ 라고 하자. 그리고 다음과 같이 정의한다.

$$
\begin{aligned}
P(\bf{x}) &:= F(\bf{x}) - F(\bf{a}) - L(\bf{x}-\bf{a}),\\[0.3em]
Q (\bf{y}) &:= G(\bf{y})- G(\bf{b})-T(\bf{y}-\bf{b}), \\[0.3em]
R(\bf{x}) &: = G\circ F(\bf{x})- G \circ F(\bf{a}) - (T\circ L) (\bf{x}-\bf{a}).
\end{aligned}
$$ {#eq-thm_CM_chainrule_1}

미분가능성의 정의로부터

$$
\lim_{\bf{x}\to\bf{a}}\dfrac{\|P(\bf{x})\|}{\|\bf{x}-\bf{a}\|} = 0, \qquad \lim_{\bf{y}\to\bf{b}}\dfrac{\|Q(\bf{y})\|}{\|\bf{y}-\bf{b}\|} = 0
$$ {#eq-thm_CM_chainrule_2}


이다. 우리는 이제 다음을 보이면 된다.

$$
\lim_{\bf{x}\to\bf{a}}\dfrac{\|R(\bf{x})\|}{\|\bf{x}-\bf{a}\|} = 0.
$$ {#eq-thm_CM_chainrule_3}

그렇다면

$$
\begin{aligned}
R(\bf{x}) &= G(F(\bf{x})) -  G(\bf{b}) - (T \circ L)(\bf{x}-\bf{a})\\[0.3em]
& = G(F(\bf{x})) - G(\bf{b}) - T(F(\bf{x}) - F(\bf{a})- P(\bf{x})) \\[0.3em]
&= \left[G(F(\bf{x})) - G(\bf{b}) -T(F(\bf{x})-F(\bf{a}))\right] + T(P(\bf{x})) \\[0.3em]
&=Q(F(\bf{x})) + T(P(\bf{x}))
\end{aligned}
$$ {#eq-thm_CM_chainrule_4}

이다. 우선 $\displaystyle \lim_{\bf{x}\to \bf{a}} \dfrac{\|T(P(\bf{x}))\|}{\|\bf{x}-\bf{a}\|}=0$ 임을 보이자. @exr-CM_spivak_1_10 으로부터 어떤 $M_T>0$ 에 대해

$$
\dfrac{\|T(P(\bf{x}))\|}{\|\bf{x}-\bf{a}\|}\le \dfrac{M_T\|P(\bf{x})\|}{\|\bf{x}-\bf{a}\|}
$$ {#eq-thm_CM_chainrule_5}

이며 @eq-thm_CM_chainrule_2 의 첫번째 극한으로부터 다음을 알 수 있다.

$$
\displaystyle \lim_{\bf{x}\to \bf{a}} \dfrac{\|T(P(\bf{x}))\|}{\|\bf{x}-\bf{a}\|}=0
$$ {#eq-thm_CM_chainrule_6}

또한 @eq-thm_CM_chainrule_2 의 두번째 극한으로부터 임의의 $\varepsilon>0$ 에 대해 다음을 만족하는 $\delta>0$ 이 존재한다는 것을 안다.

$$
\|F(\bf{x})-\bf{b}\|<\delta \implies \|Q(F(\bf{x}))\| < \varepsilon \|F(\bf{x})-\bf{b}\|
$$

그렇다면 @eq-thm_CM_chainrule_1 로부터

$$
\begin{aligned}
\varepsilon \|F(\bf{x})-\bf{b}\| &= \varepsilon \| P(\bf{x}) + L(\bf{x}-\bf{a})\| \\[0.3em]
& \le \varepsilon \|P(\bf{x})\| + \varepsilon\|L(\bf{x}-\bf{a})\|
\end{aligned}
$$

이며 역시  @exr-CM_spivak_1_10 로부터 어떤 $M_L>0$ 가 존재하여 $\|L(\bf{x}-\bf{a})\| \le M_L \|\bf{x}-\bf{a}\|$ 임을 안다. 이 결과와 @eq-thm_CM_chainrule_2 의 첫번째 식을 이용하면 어떤 $\eta>0$ 에 대해

$$
\|\bf{x}-\bf{a}\|<\eta \implies \|P(\bf{x})\| <\|\bf{x}-\bf{a}\|
$$

이므로 임의의 $\varepsilon>0$ 에 대해 어떤 $\eta>0$ 이 존재하여 

$$
\|\bf{x}-\bf{a}\|<\eta \implies \|Q(F(\bf{x}))\| \le \varepsilon(M_L+1) \|\bf{x}-\bf{a}\|
$$

이다. 임의의 $\varepsilon>0$ 에 대해 성립하므로 

$$
\lim_{\bf{x}\to \bf{a}}\dfrac{\|Q(F(\bf{x}))\|}{\|\bf{x}-\bf{a}\|} = 0
$${#eq-thm_CM_chainrule_7}


이다. 따라서 @eq-thm_CM_chainrule_4 로 부터

$$
\lim_{\bf{x}\to \bf{a}} \dfrac{\|R(\bf{x})\|}{\|\bf{x}-\bf{a}\|} =0
$$

이므로 $(G\circ F)'(\bf{a}) = T\circ L= (G\circ F)'(\bf{a}) \cdot F'(\bf{a})$ 이다. $\square$
:::
...
::: {#prp-CM_basic_properties_of_derivative_of_multivariable_functions}

$F:\R^n \to \R^m$ 에 대해 다음이 성립한다.

($1$) $F$ 가 상수함수이면 모든 $\bf{x}\in \R^n$ 에 대해 $F'(\bf{x}) = \bf{0}$ 이다.
...
$F:\R^n \to \R^m$, $F(\bf{x}) = \begin{bmatrix}f_1(\bf{x}) &\cdots & f_m(\bf{x})\end{bmatrix}^T$ 이라고 일 때 $F$ 가 $\bf{a}$ 에서 미분 가능한 필요충분조건은 각각의 $f_i$ 가 $\bf{a}$ 에서 미분가능한 것이며 
...
::: {.proof}
우선 $f_i'(\bf{a})$ 는 $1 \times n$ 행렬, 즉 열행렬임을 안다. $1 \times n$ 열행렬 $\bf{a}_1,\ldots,\, \bf{a}_m$ 에 대해 
...
이며 $\bf{h}\to \bf{0}$ 극한에서 우변이 $0$ 이므로 좌변도 $0$ 이다. 즉 $F'(\bf{a}) = T$ 이다. 

이제 $F$ 가 $\bf{a}$ 에서 미분가능하다고 하자. 사영함수(@def-CM_projection_function) $\pi_i$ 에 대해 @exr-CM_differentiability_of_identity_function 에서 미분가능함을 보였다. @thm-CM_chain_rule 에 의해 $\pi_i \circ F$ 도 $\bf{a}$ 에서 미분가능하다. $\square$
:::

</br><div class="border" style="background-color:#F2F4F4  ;padding:5px;">

::: {#exm-CM_simplle_derivative_examples}

($1$) $s:\R^2 \to \R$ 이 $s(x,\,y) = x+y$ 이면 $s'(x,\,y) = s$ 임은 @prp-CM_basic_properties_of_derivative_of_multivariable_functions 의 ($2$) 에 의해 자명하다.

($2$) $p:\R^2 \to \R$ 이 $p(x,\,y) = xy$ 일 때 $p'(x,\,y) = \begin{bmatrix} b \\ a\end{bmatrix}$ 임을 보이자. $L =  \begin{bmatrix} b \\ a\end{bmatrix}$ 에 대해

$$
\begin{aligned}
\lim_{(h,\,k) \to \bf{0}} \dfrac{|p(a+h,\,b+k) - p(a,\,b) - L(h,\,k)|}{\sqrt{h^2+k^2}} &= \lim_{(h,\,k) \to \bf{0}}\dfrac{|hk|}{\sqrt{h^2+k^2}} \\[0.3em]
&\le \lim_{(h,k)\to \bf{0}}\dfrac{h^2+k^2}{2\sqrt{h^2+k^2}} =0
\end{aligned}
$$
...
$$
\begin{aligned}
(f+g)'(\bf{a})  &= f'(\bf{a}) + g'(\bf{a}), \\[0.3em]
(f\cdot g)'(\bf{a}) &= g(\bf{a}) f'(\bf{a}) + f(\bf{a}) g'(\bf{a}).
\end{aligned}
$$ {#eq-cor_CM_basic_properties_of_derivative_of_multivariable_functions_1}

또한 $g(\bf{a})\ne 0$ 이면 다음이 성립한다.

$$
(f/g)'(\bf{a}) = \dfrac{g(\bf{a})f'(\bf{a}) - f(\bf{a})'g(\bf{a})}{[g(\bf{a})]^2} 
$$ {#eq-cor_CM_basic_properties_of_derivative_of_multivariable_functions_2}
...
@exm-CM_simplle_derivative_examples ($1$) 의 $s(x,\,y)=x+y$ 를 이용한다. $f+g = s\circ (f,\,g)$ 이므로

$$
\begin{aligned}
(f+g)'(\bf{a}) &= s'(f(\bf{a}),\, g(\bf{a})) \circ (f,\,g)'(\bf{a}) \\[0.3em]
&= s \circ ((f,\,g)'(\bf{a}) ) = f'(\bf{a}) + g'(\bf{a})
\end{aligned}
$$

이다. 

@exm-CM_simplle_derivative_examples ($2$) 의 $p(x,\,y) = x\cdot y$ 를 이용한다. $f\cdot g = p\circ(f,\,g)$ 이므로

$$
\begin{aligned}
(f \cdot g)'(\bf{a}) &= p'(f(\bf{a}),\, g(\bf{a}))\circ (f,\, g)'(\bf{a})\\[0.3em]
&= g(\bf{a})f'(\bf{a}) + f(\bf{a})'g(\bf{a})
\end{aligned}
$$

이다. 

이제 @eq-cor_CM_basic_properties_of_derivative_of_multivariable_functions_2 를 보이기 위해 $g(\bf{a})\ne 0$ 이며 $\bf{a}$ 에서 미분가능할 때 $(1/g)'(\bf{a})$ 를 구해보자. $h(x) = 1/g(x)$ 라고 하면 $g\cdot h = 1$ 이며 $(1)'(\bf{x})=0$ 이므로

$$
0 = g(\bf{a})h'(\bf{a}) + h(\bf{a})g'(\bf{a})
$$

이며 따라서 

$$
(1/g)'(\bf{a})= - \dfrac{g'(\bf{a})}{[g(\bf{a})]^2}
$$
...
$$
\begin{aligned}
(f/g)'(\bf{a}) &= (f\cdot h)'(\bf{a}) = \dfrac{f'(\bf{a})}{g(\bf{a})} - \dfrac{f(\bf{a}) g'(\bf{a})}{[g(\bf{a})]^2} \\[0.3em]
&= \dfrac{g(\bf{a})f'(\bf{a})-f(\bf{a})g'(\bf{a}) }{[g(\bf{a})]^2}
\end{aligned}
$$
...
($b$) $F'(\bf{a},\,\bf{b})(\bf{x},\,\bf{y}) = F(\bf{a},\,\bf{y}) + F(\bf{x},\, \bf{b})$ 임을 보여라.

($c$) @exm-CM_simplle_derivative_examples ($2$) 의 $Dp(a,\,b)$ 는 ($b$) 의 특별한 형태임을 보여라.
...
$F:\R^n \times \R^m \to \R^p$ 가 쌍선형 형식일 때 다음이 성립한다.

&emsp;($1$) $F(\bf{0},\,\bf{y}) = F(\bf{x},\,\bf{0})=\bf{0}$ 이다.
...
이다. 미분의 정의에 의해 $F'(\bf{x},\,\bf{y}) (\bf{h,\, \bf{k}})= F(\bf{x},\, \bf{k}) + F(\bf{h},\, \bf{y})$ 이다.
...
$$
\begin{aligned}
F'(a_1,\ldots,\,a_k)&(x_1,\ldots,\,x_k) = \sum_{i=1}^k F(a_1,\ldots,\,a_{i-1},\, x_i,\,a_{i+1},\ldots,\,a_k)
\end{aligned}
$$
...
이제 주어진 $\displaystyle F'(a_1,\ldots,\,a_k)(h_1,\,\ldots,\,h_k) = \sum_{i=1}^k F(a_1,\ldots,\,a_{i-1},\, h_i,\,a_{i+1},\ldots,\,a_k)$ 의 우변은 $F(a_1,\ldots,\,a_k)$ 의 $k$ 개의 인자 $a_1,\ldots,\,a_k$ 가운데 하나만 $\bf{h}$ 의 해당 성분으로 바꾼 것이다. 그렇다면
...
### 편미분의 정의 {#sec-CM_partial_derivative}
...
$$
D_i f(\bf{a}) :=\lim_{h \to 0} \dfrac{f(\bf{a}+\bf{e}_i h) - f(\bf{a})}{h}.
...
### 편미분의 기본 성질 {#sec-CM_basic_properties_of_partial_derivative}
...
$f:\R \to \R$ 을 다음과 같이 정의한다.

$$
f(x) := \left\{\begin{array}{ll}e^{-(x-1)^{-2}} \cdot e^{-(x+1)^{-2}} , \qquad & x\in (-1,\,1) \\ 0, & x\notin (-1,\,1).\end{array}\right.
$$

($a$) $f$ 가 $C^\infty$ 급 함수이며 $(-1,\,1)$ 에서 양수이고 다른 곳에선 $0$ 임을 보여라.

($b$) $x\le 0$ 일 때는 $0$ 이며 $x\ge \varepsilon$ 일 때는 $1$ 인 $C^\infty$ 함수 $g:\R\to [0,\,1]$ 가 존재함을 보여라.

($c$) $\bf{a}\in \R^n$ 에 대해 $g:\R^n \to \R$ 이 아래와 같이 정의되었다.

$$
g(\bf{x}) := f\left(\dfrac{x_1-a_1}{\varepsilon}\right) \cdots f\left(\dfrac{x_n-a_n}{\varepsilon}\right).
$$

$g$ 는 $(a_1-\varepsilon,\,a_1+\varepsilon) \times \cdots \times (a_n-\varepsilon,\, a_n+\varepsilon)$ 에서 양의 값을, 그 외는 $0$ 을 반환하는 함수임을 보여라.

($d$) 열린 $A\subset \R^n$ 와 옹골집합인 $C\subset A$ 에 대해 어떤 non-negative $C^\infty$ 함수 $f:A \to \R$ 가 존재하여 $x\in C$ 에서는 $f(x)>0$ 이고 $A$ 에 포함된 닫힌 집합 밖에서는 $0$ 일 수 있음을 보여라.

($e$) $f:A\to [0,\,1]$ 을 $x\in C$ 에서 $f(x)=1$ 이 되도록 할 수 있음을 보여라.

:::
</div>
::: {.solution}

($a$) 우선 $k \in \Z_+$ 에 대해 로피탈의 정리(@thm-MA_Lhopital_theorem) 를 사용하여 다음을 보일 수 있다.

$$
\lim_{t \to 0} \dfrac{e^{-1/t^2}}{t^k} = \lim_{s \to +\infty} \dfrac{s^k}{e^{s^2}} = 0
$$

이다. $f$ 에 대한 도함수는 
$$
f'(x) =  \dfrac{2}{(x-1)^3} f(x) + \dfrac{2}{(x+1)^3}f(x)
$$

이며 따라서 $\lim_{x \to \pm 1} f^{(n)}(x) = 0$ 이다. $x\ne \pm 1$ 에서는 $f^{(n)}$ 이 모두 정해지므로 $f$ 는 $C^\infty$ 급 함수이다. $x\in (-1,\,1)$ 에서는 양수이며 다른곳에서는 $0$ 임은 정의에 의해 자명하다.

($b$) 정해진 $\varepsilon>0$ 에 대해 $\overline{g}_\varepsilon:\R\to \R$ 을

$$
\overline{g}_\varepsilon(x) := \left\{\begin{array}{ll} e^{-(x-\varepsilon)^{-2}} \cdot e^{-x^{-2}} , \qquad & x\in (0,\,\varepsilon) \\ 0, & x\notin (0,\,\varepsilon).\end{array}\right.
$$

으로 정의하고 $g:\R \to \R$ 을 

$$
g(x) := \left(\int_0^x \overline{g}_\varepsilon\right) / \left(\int_0^\varepsilon \overline{g}_\varepsilon\right)
$$

으로 정의하자. $\overline{g}_\varepsilon$ 이 $C^\infty$ 함수이다. 또한 단조증가 함수이므로 $g$ 역시 $C^\infty$ 급 함수이며 주어진 조건을 만족한다. 

($c$) ($a$) 에 의해 자명하다.

($d$) $\Gamma$ 를 $C$ 의 각 점에 대해 $A$ 에 포함되는 열린 $n$-입방체 의 집합이라고 하자. $\Gamma$ 는 $C$ 에 대한 열린 덮개이므로 유한개의 열린 입방체로 $C$ 를 덮을 수 있다. 이를 $\{U_1,\ldots,\,U_m\}$ 이라고 하자. $U_i$ 에 대해 ($c$) 의 $g$ 함수를 적용한 것을 $g_i$ 라고 하고 $f = g_1+ \cdots + g_n$ 이라고 하자. 그렇다면 $x\in C$ 에 대해 $f(x)>0$ 임은 자명하다. 이제 $U_i$ 에 대해 $\overline{U_i}\subset A$ 이며 $X=\bigcup_{i=1}^m \overline{U_i} \subset A$ 는 닫힌집합이며 $\bf{x}\in X$ 일 때 $f(\bf{x})=0$ 이다.

($e$) $h:A \to \R$ 을 ($d$) 의 $f$ 로 놓고 $\bf{x}\in C$ 일 때 $h(\bf{x})\ge \varepsilon$ 이 되도록 놓을 수 있다. 이제 ($b$) 의 $g$ 에 대해 $f=g\circ h$ 로 놓으면 된다.
...
@thm-CM_basic_properties_of_derivative_of_multivariable_functions 에 의해 $f_i'(\bf{a}) = \begin{bmatrix} D_1 f_i(\bf{a}) & \cdots D_n f_i(\bf{a})\end{bmatrix}$ 임을 보이면 된다. $H:\R^n \to \R^n$ 를 $\bf{a}$ 에서 $j$ 번째 성분만 $x$ 인 함수라고 하자. 즉
...
즉 우리는 $F:\R^n \to \R^m$ 이 $\bf{a}$ 에서 미분가능할경우 $F'(\bf{a})$ 를 $D_jf_i(\bf{a})$ 로 표현 가능하다는 것을 알았다. 자코비 행렬(@def-CM_jacobian) 를 생각하면 $J_F(\bf{a}) = F'(\bf{a})$ 이다. 이 이외에도 아래와 같은 여러가지 표기법이 존재한다. 

$$
J_F(\bf{a}) = F'(\bf{a}) = \dfrac{\partial (f_1,\ldots,\,f_m)}{\partial (x_1,\ldots,\,x_n)} = \begin{bmatrix} D_1f_1(\bf{a}) & \cdots & D_nf_1(\bf{a})\\ \vdots & & \vdots \\ D_1 f_m(\bf{a}) & \cdots & D_n f_m(\bf{a}) \end{bmatrix}
$$




@thm-CM_derivative_and_partial_derivative_1 에 대한 역은 성립하지 않는데 $\bf{a}$ 에 대해 $F$ 의 모든 편미분이 존재하더라도 $F$ 가 미분가능하지 않을 수 있다. 여기에는 하나의 조건이 더 필요하다.
...
$$ {#eq-thm_CM_mean_value_theorem_for_vector_function}
...
인 $t_0\in (0,\,1)$ 이 존재하며 이는 @eq-thm_CM_mean_value_theorem_for_vector_function 를 의미한다. $\square$

:::

</br>

집합 $E\subset \R^n$ 가 각각의 $\bf{x},\,\bf{y}\in E$ 에 대해 $\bf{x},\,\bf{y}$ 를 잇는 선분의 모든 점이 $E$ 에 포함될 때 $E$ 를 볼록 집합(@def-MA_convex) 이라고 한다.


<div class="border" style="background-color:#FFF0F5 ;padding:5px;">

::: {#cor-CM_mean_value_theorem_for_convex}

#### 볼록집합에서의 중간값 정리

열린 볼록집합 $U\subset\R^n$ 에서 정의된 $F:U \to \R$ 가 $U$ 에서 미분가능하면 $\bf{x},\,\bf{y}\in U$ 일 때 다음을 만족하는 $\bf{c}\in U$ 가 존재한다.

$$
F(\bf{x})-F(\bf{y})= F'(\bf{c})\cdot (\bf{x}-\bf{y}).
$${#eq-cor_CM_mean_value_theorem_for_convex}
