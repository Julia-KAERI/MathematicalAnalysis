...
$L=F'(\bf{a})$ 라고 하자. @exr-part3_spivak_1_10 에서 보았듯이 모든 $\bf{x}\in \R^n$ 에 대해 $\|L\bf{x}\|\le M_1\|\bf{x}\|$ 을 만족하는 $M_1$ 이 존재하며 $\det (L)\ne 0$ 이므로 $\bf{x}\ne \bf{0}$ 이면 $L\bf{x}\ne \bf{0}$ 이다. 따라서 $M_1>0$ 이다. 그렇다면 역시 $\|L^{-1}\bf{x}\|\le M_2\|\bf{x}\|$ 인 $M_2>0$ 도 존재한다.
...
($C-2$) $\det$ 함수와 $F'$ 이 모두 연속이므로 $\bf{a}$ 를 중심으로 하는 어떤 열린 cube $U_2\subset A$ 에서 $\bf{x}\in U_2 \implies \det(DF(\bf{x})) \ne 0$ 이다. 
...
$$
\bf{0}=Dg(\bf{x}) = 2 DF(\bf{x})\bf{\cdot} (\bf{y}-F(\bf{x}))
$$

이다. 그런데 $\det(DF(\bf{x})) \ne 0$ 이므로 $\bf{y}=F(\bf{x})$ 이어야 한다. 즉 $\bf{y}=F(\bf{x})$ 인 $\bf{x}\in U$ 가 존재한다. $U\subset U_0$ 이므로 ($C-1$) 에 의해 $U$ 에서 유일하다. 
