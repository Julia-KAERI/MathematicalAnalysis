...
### 다변수 함수 미분의 기본 성질 {#sec-part3_basic_properties_of_derivative_of_multivariable_functions}

</br>

<div class="border" style="background-color:#FFF0F5 ;padding:5px;">

::: {#thm-part3_basic_properties_of_derivative_of_multivariable_functions}

$\bf{f}:\R^n \to \R^m$, $\bf{f}(\bf{x}) = \begin{bmatrix}f_1(\bf{x}) &\cdots & f_m(\bf{x})\end{bmatrix}^T$ 이라고 하자.
...
$$
\bf{f}'(\bf{a}) = \begin{bmatrix} f_1'(\bf{a}) & \cdots &f_m'(\bf{a})\end{bmatrix}^T
$$
...
($1$) $\bf{f}$ 가 상수함수이면 $\|\bf{f}(\bf{x}+\bf{h})- \bf{f}(\bf{x})\|=0$ 이다. 

($2$) $\bf{f}$ 가 선형사상이면 $\|\bf{f}(\bf{x}+\bf{h})- \bf{f}(\bf{x})- \bf{f}(\bf{h})\| = 0$ 이다. 
...
::: {#prp-part3_multilinear_1}

유클리드 공간 $E_1,\ldots,\,E_n$ 에 대해 $\bf{f}:E_1 \times \cdots \times E_n \to \R^p$ 가 $n$-선형 형식일 때 다음이 성립한다.

&emsp;($1$) $\bf{f}(0,\ldots,0) = \bf{0}$ 이다.

&emsp;($2$) 어떤 $M>0$ 이 존재하여 $\|\bf{f}(x_1,\ldots,\,x_n)\| \le M\|(x_1,\ldots,\,x_n)\|^n$ 이다.
:::
...
($1$) 은 @prp-part3_bilinear_1 과 같은 방법으로 쉽게 증명 할 수 있다.  

($2$) $E_i$ 의 차원이 $d_i$ 라고 하고 $\left(\bf{e}^{(i)}_j\right)$ 가 $E_i$ 의 표준기저라고 하자. $x_i = \sum_{j_i} c_{j_i}^{(i)}\bf{e}_{j_i}^{(i)}$ 라고 하면 

$$
\begin{aligned}
\|\bf{f}(x_1,\ldots,\,x_n)\| &= \left\|\bf{f}\left(\sum_{j_1} c^{(1)}_{j_1} \bf{e}^{(1)}_{j_1}, \ldots,\, \sum_{j_n} c^{(n)}_{j_n} \bf{e}^{(n)}_{j_n}\right) \right\| \\[0.3em]
&= \sum_{j_1,\ldots,\,j_n} \left|c^{(1)}_{j_1} \cdots c^{(n)}_{j_n}\right| \bf{f}\left(\bf{e}^{(1)}_{j_1},\ldots,\, \bf{e}^{(n)}_{j_n}\right)\\[0.3em]
& \le \sum_{j_1,\ldots,\,j_n} \|\bf{x}_1\|\cdots \|\bf{x}_n\| \bf{f}\left(\bf{e}^{(1)}_{j_1},\ldots,\, \bf{e}^{(n)}_{j_n}\right)\\[0.3em]
\end{aligned}
$$

$M_0 = \max \left\{\left\| \bf{f}\left(\bf{e}^{(1)}_{j_1},\ldots,\, \bf{e}^{(n)}_{j_n}\right)\right\|\right\}$ 라고 하자. 또한
...
$$
\|\bf{f}(x_1,\ldots,\,x_n)\| \le d_1 \cdots d_n M_0 \|(\bf{x}_1,\ldots,\,\bf{x}_n)\|^n
$$
...
$$
\begin{aligned}
&\bf{f}(a_1+h_1,\ldots, a_k +h_k) - \bf{f}(a_1,\ldots,\,a_k) \\[0.3em]
&\quad = \bf{f}(a_1,\,a_2+h_2,\ldots,\,a_k+h_k) + \bf{f}(h_1,\,a_2+h_2,\ldots,\,a_k+h_k ) - \bf{f}(a_1,\ldots,\,a_k)\\[0.3em]
&\quad = \bf{f}(a_1,\,a_2,\,a_3+h_3,\ldots,a_k+h_k) + \bf{f}(a_1,\,h_2,\,a_3+h_3,\ldots,,a_k+h_k) \\[0.3em]
&\qquad\qquad \qquad + \bf{f}(h_1,a_2,\,a_3+h_3,\ldots,\,a_k+h_k) + \bf{f}(h_1,\,h_2,\,a_3+h_3,\ldots,\,a_k+h_k) \\[0.3em]
&\qquad\qquad\qquad - \bf{f}(a_1,\ldots,\,a_k)
\end{aligned}
$$

이며 이것을 계속 반복하다보면 $\bf{f}$ 의 인자로 주어지는 $k$ 개의 coordinate index 가운데 $\bf{h}$ 의 성분이 1개 있는 것부터 $k$ 개 있는 것까지의 합이 된다. 

이제 주어진 $\displaystyle D\bf{f}(a_1,\ldots,\,a_k)(h_1,\,\ldots,\,h_k) = \sum_{i=1}^k \bf{f}(a_1,\ldots,\,a_{i-1},\, h_i,\,a_{i+1},\ldots,\,a_k)$ 의 우변은 $\bf{f}(a_1,\ldots,\,a_k)$ 의 $k$ 개의 인자 $a_1,\ldots,\,a_k$ 가운데 하나만 $\bf{h}$ 의 해당 성분으로 바꾼 것이다. 그렇다면

$$
\lim_{\bf{h}\to \bf{0}}\dfrac{\displaystyle \left\|\bf{f}(a_1+h_1,\ldots,\,a_k+h+k) - \bf{f}(a_1,\ldots,\, a_k) - \sum_{i=1}^n \bf{f}(a_1,\ldots,\,a_{i-1},h_i,a_{i+1},\ldots,a_k)\right\|}{\|\bf{h}\|}
$$

를 생각해보자. 분자의 $\| \cdots \|$ 안에는 $\bf{f}$ 의 인자 $a_1,\ldots,\,a_k$ 가운데 두개 이상을 $\bf{h}$ 의 해당 성분으로 바꾼 것만 남는다. 이제 우리가 증명한 @prp-part3_multilinear_1 에 의해 위의 극한이 $0$ 이 됨을 알 수 있다. 
...
($a$) 우리는 $n \times n$ 행렬의 행렬식이 $n$-선형 형식임을 안다. 따라서 @exr-part3_spivak_2_14 ($b$) 에 의해 주어진 식이 성립함을 안다.
...
::: {#exr-part3_spivak_2_16}
#### Spivak 2_16

$\bf{f}:\R^n \to \R^n$ 이 미분가능하며 미분가능한 역함수 $\bf{f}^{-1}:\R^n \to \R^n$ 이 존재한다고 하자. 다음이 성립함을 보여라.

$$
(\bf{f}^{-1})'(\bf{a}) = \left[\bf{f}'\left(\bf{f}^{-1}(\bf{a})\right)\right]^{-1}
$$
...
$\bf{f}\circ \bf{f}^{-1} = \bf{\pi}$ 이며 양변을 미분하면 $\bf{f}'(\bf{f}^{-1}(\bf{a})) (\bf{f}^{-1})'(\bf{a}) = \bf{I}_n$ 이므로 주어진 식이 성립한다. 
:::
...
$$ {#eq-part3_ith_partial_derivative}
...
::: {#thm-part3_exchange_of_the_order_of_partial_derivative}
...
::: {#thm-part3_extrema_and_partial_derivative}
...
### 다변수 벡터함수의 미분과 편미분 {#sec-part3_derivative_and_partial_derivatives_of_multivariable_vector_valued_function}

<div class="border" style="background-color:#FFF0F5 ;padding:5px;">

::: {#thm-part3_derivative_and_partial_derivative_1}

#### 다변수 벡터함수의 미분과 편미분 1

$\bf{f}:\R^n \to \R^m$ 이 $\bf{a}\in \R^n$ 에서 미분가능하다면 각각의 $1 \le i \le m,\, 1 \le j \le n$ 에 대해 $D_jf_i(\bf{a})$ 가 존재하며, $\bf{f}'(\bf{a})$ 는 $m\times n$ 행렬로 $D_jf_i(\bf{a})$ 를 $i$ 행 $j$ 열 성분으로 가진다. 즉

$$
\bf{f}'(\bf{a}) = \begin{bmatrix} D_1f_1(\bf{a}) & \cdots & D_nf_1(\bf{a})\\ \vdots & & \vdots \\ D_1 f_m(\bf{a}) & \cdots & D_n f_m(\bf{a}) \end{bmatrix}
$$ 
...
@thm-part3_basic_properties_of_derivative_of_multivariable_functions 의 ($3$) 에 의해 $f_i'(\bf{a}) = \begin{bmatrix} D_1 f_i(\bf{a}) & \cdots D_n f_i(\bf{a})\end{bmatrix}$ 임을 보이면 된다. $\bf{h}:\R \to \R^n$ 를 $\bf{a}$ 에서 $j$ 번째 성분만 $x$ 인 함수라고 하자. 즉

$$
\bf{h}(\bf{a}) = (a_1,\ldots,\,a_{j-1},\,x,\,a_{j+1},\ldots,\,a_n)
$$

이다. 그렇다면 $D_j f_i(\bf{a}) = (f_i \circ \bf{h})'(a_j) = f_i'(\bf{a}) \cdot \bf{h}'(a_j) =f'_i(\bf{a})\cdot \bf{e}_j= (f'_i(\bf{a}))_j$ 이다. 따라서 
...
즉 우리는 $\bf{f}:\R^n \to \R^m$ 이 $\bf{a}$ 에서 미분가능할경우 $\bf{f}'(\bf{a})$ 를 $D_jf_i(\bf{a})$ 로 표현 가능하다는 것을 알았다. 그러나 그 역은 성립하지 않는데 $\bf{a}$ 에 대해 $\bf{f}$ 의 모든 편미분이 존재하더라도 $\bf{f}$ 가 미분가능하지 않을 수 있다. 여기에는 하나의 조건이 더 필요하다.

</br><div class="border" style="background-color:#FFF0F5 ;padding:5px;">

::: {#thm-part3_derivative_and_partial_derivative_2}

#### 다변수 벡터함수의 미분과 편미분 2

$\bf{f}:\R^n \to \R^m$ 에 대해 $\bf{a}$ 를 포함하는 어떤 열린 공간에서 $D_jf_i(\bf{a})$ 가 존재하며 연속이면 $\bf{f}$ 는 $\bf{a}$ 에서 미분가능하다.
::: 
...
$f_i = \pi_i\circ \bf{f}$ 라고 하자. 또한 $\bf{a}$ 를 포함하는 위의 조건을 만족하는 열린 공간에서만 생각한다. 
...
$$
\begin{aligned}
&\left\|\bf{f}(\bf{a}+\bf{h})-\bf{f}(\bf{a}) - \sum_{i=1}^m \sum_{j=1}^n D_j f_i(\bf{a}) \cdot h_j\right\| \\[0.3em] 
&\qquad \qquad \qquad=\sum_{i=1}^m \left| f_i(\bf{a}+\bf{h})-f_i(\bf{a}) - \sum_{j=1}^n D_j f_i(\bf{a}) h_j\right| \\[0.3em]
...
$$
\lim_{\bf{h}\to \bf{0}} \dfrac{\displaystyle \left\|\bf{f}(\bf{a}+\bf{h})-\bf{f}(\bf{a}) - \sum_{i=1}^m \sum_{j=1}^n D_j f_i(\bf{a}) \cdot h_j\right\| }{\|\bf{h}\|} \le \lim_{\bf{h}\to \bf{0}}\sum_{i=1}^m \sum_{j=1}^n\left|D_j f_i(\bf{c}_j) - D_j f_i (\bf{a})\right| = 0
$$
...
::: {#thm-part3_chain_rule_for_multivariable_vector_valued_function}

#### 다변수 벡터함수의 연쇄 규칙

$\bf{g}:\R^n \to \R^m$ 이 $C^1$ 급 함수이고 $\bf{f}:\R^m \to \R^p$ 이 $\bf{g}(\bf{a})$ 에서 미분가능 할 때 다음이 성립한다.

$$
D(\bf{f} \circ \bf{g})(\bf{a}) = D\bf{f}(\bf{g})\cdot D\bf{g}(\bf{a})
$$
