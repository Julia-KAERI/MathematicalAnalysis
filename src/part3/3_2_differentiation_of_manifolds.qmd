...
$F:\R^n \times \R^m \to \R^n \times \R^m$ 이 $F(\bf{x},\,\bf{y}) = (\bf{x},\, F_1(\bf{x,\,y}))$ 꼴이며 역함수가 존재한다면 $F^{-1}(\bf{x},\,\bf{y}) = (\bf{x},\, F_2(\bf{x}))$ 꼴임을 보이자. $F^{-1}=G=\begin{bmatrix} g_1 & \cdots & g_{n+m}\end{bmatrix}^T$ 라고 하자. 
...
$V$ 를 $A\subset \R^n,\, B\subset \R^m$ 에 대해 $V=A \times B$ 가 되도록 할 수 있으며 그렇다면 $\widetilde{F}^{-1}: W \to A \times B$ 는 어떤 함수 미분가능한 함수 $K:W \to B$ 에 대해 $\widetilde{F}^{-1}(\bf{x},\,\bf{y}) = (\bf{x},\, K(\bf{x},\, \bf{y}))$ 이다. 이제 $\Phi:\R^n \times \R^m \to \R^m$ 을 $\Phi (\bf{x},\, \bf{y}) := \bf{y}$ 로 정의하자. $\Phi \circ \widetilde{F} = F$ 이다. 따라서
...
$F:\R^n \to \R^p$ 가 $\bf{a}\in R^n$ 을 포함하는 열린 집합에서 $C^1$ 급 함수이고 $p\le n$ 이라고 하자. $F(\bf{a})=\bf{0}$ 이며 $p\times n$ 행렬 $M_{ij} = D_{j}f_i(\bf{a})$ 에 대해 $\text{rank}(M)=p$ 라면 $\bf{a}$ 를 포함하는 어떤 열린 집합 $A\subset \R^n$ 에서 정의된, 아래의 ($1$), ($2$) 를 만족하는 $H: A\to \R^n$ 이 존재한다. 


&emsp;($1$) $H^{-1}$ 이 존재하고 미분가능하며,

&emsp;($2$) $(F\circ H)(x_1,\ldots,\,x_n) = (x^{n-p+1},\ldots,\,x_n)$ 이다.
...
$F$ 를 $F:\R^{n-p}\times \R^p \to \R^p$ 로 생각하고 @thm-part3_implicit_function_theorem_1 의 증명과정을 다시 보자. $H:A \to \R^n$ 을 $H(\bf{x},\, \bf{y})= (\bf{x},\, K(\bf{x},\,\bf{y}))$ 라고 놓으면 

$$
(F\circ H)(\bf{x},\,\bf{y}) = \bf{y}
$$
