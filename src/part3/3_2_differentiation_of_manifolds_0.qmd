...
이다. 우선 $\displaystyle \lim_{\bf{x}\to \bf{a}} \dfrac{\|T(\bf{\varphi}(\bf{x}))\|}{\|\bf{x}-\bf{a}\|}=0$ 임을 보이자. @exr-part3_spivak_1_10 으로부터 어떤 $M_T>0$ 에 대해
...
이며 역시  @exr-part3_spivak_1_10 로부터 어떤 $M_L>0$ 가 존재하여 $\|L(\bf{x}-\bf{a})\| \le M_L \|\bf{x}-\bf{a}\|$ 임을 안다. 이 결과와 @eq-thm_part3_chainrule_2 의 첫번째 식을 이용하면 어떤 $\eta>0$ 에 대해
...
::: {#thm-part3_mean_value_theorem_for_vector_function}
