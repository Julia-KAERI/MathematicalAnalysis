...
$F:\R^n \to \R^m$ 이 $\bf{a}\in \R^n$ 에서 미분가능하다면 @eq-part3_derivative_of_vector_function 를 만족하는 선형변환은 유일하다. 
...
$T,\, L\in \mathcal{L}(\R^n,\, \R^m)$ 이 @eq-part3_derivative_of_vector_function 를 만족한다고 하자. 그리고 $D(\bf{h}) = F(\bf{x}-\bf{a})-F(\bf{a})$ 라고 하자. 
...
$F:\R^n\to\R^m$ 이 모든 $\bf{x}\in \R^n$ 에서 미분가능하면 마치 일변수 함수의 $f'(x)$ 처럼 $DF(\bf{x})$ 와 같이 쓸 수 있다. 이 때는 $\bf{x}$ 를 제외하고 $DF$ 와 같이 표기 할 수 있다.

$F:\R^n \to \R^m$ 이 $\bf{a}\in \R^n$ 에서 미분가능 할 때의 $DF(\bf{a})$ 는 선형변환이므로 행렬로 표현할 때 기저에 따라 그 성분값이 달라진다. 여기에 기준이 되는 야코비 행렬을 아래와 같이 정의한다.
...
@eq-part3_derivative_of_vector_function 로부터 $\displaystyle \lim_{\bf{h}\to \bf{0}} \|F(\bf{a}+\bf{h}) - F(\bf{a}) - T(\bf{h})\| = 0$ 이며 @exr-part3_spivak_1_10 로부터 $\displaystyle \lim_{\bf{h} \to \bf{0}} T(\bf{h})=\bf{0}$ 이므로 $\displaystyle \lim_{\bf{h}\to \bf{0}} F(\bf{a}+\bf{h})-F(\bf{a}) = \bf{0}$ 이다. 즉 $F$ 는 $\bf{a}$ 에서 연속이다.
:::
...
$\bf{b}=F(\bf{a})$, $T=DF(\bf{a}),\, L = DG(F(\bf{a}))$ 라고 하자. 그리고 다음과 같이 정의한다.

$$
\begin{aligned}
\bf{\varphi} (\bf{x}) &:= F(\bf{x}) - F(\bf{a}) - T(\bf{x}-\bf{a}),\\[0.3em]
\bf{\psi} (\bf{y}) &:= G(\bf{y})- G(\bf{b})-L(\bf{y}-\bf{b}), \\[0.3em]
\bf{\rho}(\bf{x}) &: = G\circ F(\bf{x})- G \circ F(\bf{a}) - L\circ T (\bf{x}-\bf{a}).
\end{aligned}
...
$$
\begin{aligned}
\bf{\rho}(\bf{x}) &= \bf{g}(F(\bf{x})) -  \bf{g}(\bf{b}) - (L \circ T)(\bf{x}-\bf{a})\\[0.3em]
&=\bf{g}(F(\bf{x})) - bf{g}(\bf{b}) - L(F(\bf{x}) - F(\bf{a})- \bf{\varphi}(\bf{x})) \\[0.3em]
&= \left[\bf{g}(F(\bf{x})) - \bf{g}(\bf{b}) -L(F(\bf{x})-F(\bf{a})\right] + L(\bf{\varphi}(\bf{x})) \\[0.3em]
&=\bf{\psi}(F(\bf{x})) + L(\bf{\varphi}(\bf{x}))
\end{aligned}
...
$$
\begin{aligned}
\varepsilon \|F(\bf{x})-\bf{b}\| &= \varepsilon \| \bf{\varphi}(\bf{x})- T(\bf{x}-\bf{a})\| \\[0.3em]
...
$$
\|\bf{x}-\bf{a}\|<\eta \implies \|\bf{\psi}(F(\bf{x}))\| \le \varepsilon(M_T+1) \|\bf{x}-\bf{a}\|
$$
...
이므로 $(G\circ F)'(\bf{a}) = L\circ T = DG(F(\bf{a})) \circ DF(\bf{a})$ 이다. $\square$
:::
...
이제 $F$ 가 $\bf{a}$ 에서 미분가능하다고 하자. 사영함수(@def-part3_projection_function) $\pi_i$ 에 대해 @exr-part3_differentiability_of_identity_function 에서 미분가능함을 보였다. @thm-part3_chain_rule 에 의해 $\pi_i \circ F$ 도 $\bf{a}$ 에서 미분가능하다. 
...
@thm-part3_basic_properties_of_derivative_of_multivariable_functions ($4$) 의 $s(x,\,y)=x+y$ 를 이용한다. $f+g = s\circ (f,\,g)$ 이므로
...
@thm-part3_basic_properties_of_derivative_of_multivariable_functions ($5$) 의 $p(x,\,y) = x\cdot y$ 를 이용한다. $f\cdot g = p\circ(f,\,g)$ 이므로
...
($c$) @thm-part3_basic_properties_of_derivative_of_multivariable_functions ($5$) 의 $Dp(a,\,b)$ 는 ($b$) 의 특별한 형태임을 보여라.
...
($1$) @eq-part3_spivak_2_12_1 의 첫번째 식으로부터  $F(-\bf{x},\,\bf{y}) = - F(\bf{x},\, \bf{y})= F(\bf{x},\, -\bf{y})$ 임을 안다. 이를 이용하면 $F(\bf{0},\, \bf{y})= F(\bf{x},\, \bf{0})= \bf{0}$ 임을 안다. 
...
를 생각해보자. 분자의 $\| \cdots \|$ 안에는 $F$ 의 인자 $a_1,\ldots,\,a_k$ 가운데 두개 이상을 $\bf{h}$ 의 해당 성분으로 바꾼 것만 남는다. 이제 우리가 증명한 @prp-part3_multilinear_1 에 의해 위의 극한이 $0$ 이 됨을 알 수 있다. 
...
@thm-part3_basic_properties_of_derivative_of_multivariable_functions 의 ($3$) 에 의해 $f_i'(\bf{a}) = \begin{bmatrix} D_1 f_i(\bf{a}) & \cdots D_n f_i(\bf{a})\end{bmatrix}$ 임을 보이면 된다. $H:\R \to \R^n$ 를 $\bf{a}$ 에서 $j$ 번째 성분만 $x$ 인 함수라고 하자. 즉
...
::: {#exr-part3_spivak_2_30}
#### Spivak 2_30

$f$ 가 @exr-part3_spivak_2_4 의 그것과 같다. $D_\bf{x}f(0,\,0)$ 이 모든 $\bf{x}$ 에서 정의됨을 보여라. 그러나 $g\ne 0$ 이면 $D_{\bf{x}+\bf{y}}f(0,\,0) = D_\bf{x}f(0,\,0) + D_\bf{y}f(0,\,0)$ 은 모든 $\bf{x},\, \bf{y}$ 에서 성립하는 것은 아님을 보여라.
...
::: {#exr-part3_spivak_2_31}
#### Spivak 2_31

$f:\R^2 \to \R$ 이 @exr-part3_spivak_1_26 과 같이 정의되었다. $f$ 가 $(0,\,0)$ 에서 연속이 아니지만 모든 $\bf{x}\in \R^2$ 에서 $D_{\bf{x}} f(0,\,0)$ 이 존재함을 보여라.
...
$f$ 가 $(0,\,0)$ 에서 연속이 아님은  @exr-part3_spivak_1_26 에서 보였다. $D_{\bf{x}}(0,\,0) = \lim_{t \to 0}\dfrac{f(t\bf{x})}{t}=0$ 이다.
:::

</br>
<div class="border" style="background-color:#F0FFFF  ;padding:5px;">

::: {#exr-part3_spivak_2_32}
#### Spivak 2_32
...
::: {#exr-part3_spivak_2_33}
#### Spivak 2_33

@thm-part3_derivative_and_partial_derivative_2 에서 $D_1f_i(\bf{a})$ 가 연속인 조건을 제거해도 된다는 것을 보여라. ($D_2f_j$ 부터는 연속이어야 한다.)
...
::: {.solution}
@thm-part3_derivative_and_partial_derivative_2 의 증명에서 $D_1f_i$ 
