...
## Measure zero 와 Content zero {#sec-part3_measure_zero_and_content_zero}

::: {.callout-note appearance="simple" icon="false"}

::: {#def-part3_measure_zero}
...
::: {#prp-part3_meazure_zero}

$A\subset \R^n$ 이 measure zero 일 때 다음이 성립한다.

&emsp;($1$) $B\subset A$ 이면 $B$ 도 measure zero 이다.

&emsp;($2$) @def-part3_measure_zero 에서 $\mathcal{U}$ 를 닫힌 $n$-cell 의 집합이 아닌 열린 $n$-cell 의 집합라고 해도 성립한다.
...
::: {#prp-part3_content_zero}

$A\subset \R^n$ 이 content zero 일 때 다음이 성립한다.

&emsp;($1$) $A$ 는 measure zero 이다.

&emsp;($2$) @def-part3_content_zero 에서 $\mathcal{U}$ 를 닫힌 $n$-cell 의 집합이 아닌 열린 $n$-cell 의 집합라고 해도 성립한다.
...
::: {#prp-part3_content_zero_and_compact}
...
::: {#exm-part3_measure_zero_not_compact}

@prp-part3_content_zero_and_compact 의 역은 성립하지 않는다. 즉 measure zero 이지만 content zero 인 집합이 존재한다. $A$ 를 $[0,\,1]$ 사이의 유리수의 집합이라고 하자. $A\subset \Q$ 이며 $\Q$ 가 measure zero 이므로(@exm-part3_Q_is_measure_zero) $A$ 도 measure zero 이다. $A$ 가 content zero 라면 유한개의 $\{[a_1,\,b_1],\ldots,\,[a_m,\,b_m]\}$ 에 대해 $[0,\,1]\subset \bigcup_{j=1}^m [a_i,\,b_i]$ 이어야 한다. 즉 $\sum \text{vol} \ge 1$ 이므로 $A$ 는 content zero 가 아니다.
:::
...
::: {#exr-part3_spivak_3_11}

#### Spivak 3.11

$A$ 가 @exr-part3_spivak_1_18 의 $A$ 라고 하자. $\sum_{i=1}^\infty (b_i-a_i)<1$ 이면 $A$ 의 경계는 measure zero 가 아님을 보여라.
...
$A=\bigcup_{i=1}^\infty (a_i,\,b_i)$ 이다.  @exr-part3_spivak_1_18 $\partial A=[0,\,1]-A$ 이다. 
...
정해진 자연수 $n$ 에 대해 $X_n = \{x:o(f,\,x)> \frac{1}{n}\}$ 라고 하자. @prp-part3_oscillation_and_continuity 로부터 $X_n \subset X$ 이다. 

$X_n = \{x_1,\,x_2,\ldots\}$ 이고 $x_1<x_2<\cdots$ 라고 하면 @exr-part3_spivak_1_30 에 따라

$$
f(b)-f(a)>f(x_k)- f(x_1) \ge \sum_{j=2}^k f(x_j)-f(x_{j-1})> \dfrac{k-1}{n}
$$

이다. $X_n$ 이 무한집합이면 $f$ 가 유계가 아니므로 $X_n$ 은 유한집합이다. $\displaystyle X=\bigcup_{i=1}^n X_n$ 이므로 @thm-part3_meazure_zero 에 따라 $X$ 는 measure zero 이다. 
...
## 적분가능한 함수 {#sec-part3_integrable_functions}

</br>
<div class="border" style="background-color:#FFF0F5 ;padding:5px;">

::: {#lem-part3_integrability}
...
($1$) 우선 $B$ 가 measure zero 임을 가정하자 $\varepsilon>0$ 에 대해 $B_\varepsilon \subset B$ 이므로 $B_\varepsilon$ 은 measure zero 이다. @prp-part3_oscillation_and_closed_set 에 의해 $B_\varepsilon$ 은 닫힌집합이며 $B_\varepsilon \subset A$ 이므로 유계이다. 따라서 $B_\varepsilon$ 은 옹골집합이므로 content zero 이다(@prp-part3_content_zero_and_compact). 따라서 유한개의 닫힌 $n$-cell $U_1,\ldots,\,U_n$ 로 $\sum_{i=1}^n \text{vol}(U_i)<\varepsilon$ 이며 $B_\varepsilon$ 을 덮을 수 있다. 이제 $A$ 에 대한 분할 $P$ 를 $P$ 에 따라 정해지는 subcell 이 아래의 $\mathcal{S}_1$ 이나 $\mathcal{S}_2$ 중의 하나에 속하도록 잡자.(@fig-part3_integrability_1)
...
![The shaded rectagles are in $\mathcal{S}_1$](fig-part3_integrability-1.png){#fig-part3_integrability_1 width=400}
...
이제 $S\in \mathcal{S}_2$ 이면 $o(f,\bf{x})<\varepsilon$ 이므로 @lem-part3_integrability 에 따라 $P$ 의 어떤 refinement $P'$ 이 존재하여 $S\subset \mathcal{S}_2$ 일 때
...
이제 $f$ 가 적분가능함을 가정하자. $B= B_1 \cup B_{1/2} \cup \cdots$ 이므로 각각의 $B_{1/n}$ 이 meausure zero 라면 @thm-part3_meazure_zero 에 의해 $B$ 가 measure zero 임을 알게 된다. 실제로 우리는 각각의 $B_{1/n}$ 이 content zero 임을 보일 것이다. 
