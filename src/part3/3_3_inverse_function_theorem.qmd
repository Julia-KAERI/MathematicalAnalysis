...
$n$-입방체 $A\subset \R^n$ 에서 정의된 $F:A \to \R^n$ 이 $C^1$ 급이라고 하자. 어떤 양수 $M>0$ 이 존재하여 모든 $\bf{x}\in \text{int}(A)$ 에 대해 $|D_jf_i(\bf{x})|\le M$ 이면 다음이 성립한다.
...
$F(\bf{x}) = (f_1 (\bf{x}),\, \cdots,\, f_n (\bf{x}))$ 이고 $\bf{x},\,\bf{y}\in A$ 일 때 다음이 성립한다.
...
각 줄마다 편미분에 대한 중간값 정리를 사용하면 $i=1,\ldots,\,n$ 에 대해

$$
f_i(\bf{y})-f_i(\bf{x}) = \sum_{j=1}^n (y_j-x_j)D_j f_i(y_1,\ldots,\,y_{j-1},\, z_{ij},\, x_{j+1},\ldots,\,x_n)
$$

을 만족하는 $z_{ij}$ 가 $x_j$ 와 $y_j$ 사이에 존재함을 안다. 이로부터 다음을 보일 수 있다.
...
선형사상 $L\in \mathcal{L}(\R^n)$ 은 $\R^n$ 전체 에서 연속이며 미분가능하다. 또한 $L'(\bf{x}) = L$ 이므로 $L$ 은 $C^1$ 함수이다. $(L\circ F)'(\bf{x}) = L\cdot F'(\bf{x})$ 이므로 $L \circ F$ 는 미분가능하며, @prp-MA_continuity_of_composited_function 에 의해 $F$ 가 $C^1$ 함수이면 $L \circ F$ 도 $C^1$ 함수이다. $F = L^{-1}\circ (L \circ F)$ 이므로 $L\circ F$ 가 $C^1$ 함수이면 $F$ 역시 $C^1$ 함수이다.
...
이제 $\bf{a}$ 를 중심으로 $U_1 \cap U_2 \cap U_3$ 에 포함되는 열린 $n$-입방체 $U$ 를 생각하자. $U$ 를 작게 잡아 $\partial U \cap \partial U_1 = \varnothing$ 이 되도록 잡을 수 있다. 이 때 $\partial U$ 는 $U$ 의 경계점(@exr-MA_KKK_2_7_6) 이다. 
...
$F:\R^n \times \R^m \to \R^n \times \R^m$ 이 $F(\bf{x},\,\bf{y}) = (\bf{x},\, H(\bf{x,\,y}))$ 꼴이며 역함수가 존재한다면 어떤 $K:\R^n\times \R^m \to \R^m$ 에 대해 $F^{-1}(\bf{x},\,\bf{y}) = (\bf{x},\, K(\bf{x},\bf{y}))$ 꼴임을 보이자. $i\le n$ 일 때,

$$
x_i = \left((F^{-1}\circ F)(\bf{x},\bf{y})\right)_i = \left(F^{-1}(\bf{x},H(\bf{x},\bf{y}))\right)_i
$$
...
$F: \R^n \times \R^m \to \R^m$ 이 $(\bf{a}, \bf{b})$ 를 포함하는 열린 집합에서 $C^1$ 급 함수이며 $F(\bf{a}, \bf{b})=0$ 이라고 하자. $m \times m$ 행렬 $M$ 을 $M_{ij} = D_{n+j}f_i(\bf{a},\,\bf{b})$ 로 정할 때 $\det (M) \ne 0$ 이면 $\bf{a}$ 를 포함하는 어떤 열린 집합 $A\subset \R^n$ 와 $\bf{b}$ 를 포함하는 어떤 열린집합 $B\subset \R^m$ 에서 정의된 미분가능한 단사 함수 $G:A \to B$ 가 존재하여 $F(\bf{x},\, G(\bf{x})) = 0$ 이 되도록 한다.
...
$\widetilde{F}: \R^n \times \R^m \to \R^n \times \R^m$ 을 $\widetilde{F}(\bf{x},\,\bf{y}) := (\bf{x},\, F(\bf{x},\,\bf{y}))$ 로 정의하자. $\det (\widetilde{F}'(\bf{a},\, \bf{b}))$$= \det (M) \ne 0$ 이므로 역함수 정리 (@thm-CM_inverse_function_theorem) 에 따라 $(\bf{a},\,\bf{b})$ 를 포함하는 열린 집합 $V$ 와 $\widetilde{F}(\bf{a},\, \bf{b})=(\bf{a},\, \bf{0})$ 을 포함하는 열린 집합 $W$ 에서 정의된 $\widetilde{F}$ 의 $C^1$-급 역함수 $\widetilde{F}^{-1}:W \to V$ 가 존재한다.

$V$ 를 $A\subset \R^n,\, B\subset \R^m$ 에 대해 $V=A \times B$ 가 되도록 할 수 있으며 그렇다면 $\widetilde{F}^{-1}: W \to A \times B$ 는 어떤 $C^1$ 함수 $K:W \to B$ 에 대해 $\widetilde{F}^{-1}(\bf{x},\,\bf{y}) = (\bf{x},\, K(\bf{x},\, \bf{y}))$ 이다(@exm-CM_pts_of_inverse_function). 이제 $\Phi:\R^n \times \R^m \to \R^m$ 을 $\Phi (\bf{x},\, \bf{y}) := \bf{y}$ 로 정의하면 $\Phi \circ \widetilde{F} = F$ 이다. 따라서

$$
F(\bf{x},\, K(\bf{x},\, \bf{y})) = (F \circ \widetilde{F}^{-1})(\bf{x},\, \bf{y}) = (\Phi \circ \widetilde{F} \circ \widetilde{F}^{-1})(\bf{x},\, \bf{y}) = \bf{y}
$$

이며 $F(\bf{x},\, K(\bf{x},\,\bf{0})) = \bf{0}$ 이므로 $G(\bf{x}) = K(\bf{x},\,\bf{0})$ 으로 놓으면 된다. $\square$
...
$\bf{x}\in \R^n,\, \bf{y}\in \R^m$ 을 $(\bf{x},\, \bf{y}) = (x_1,\ldots,\,x_n,\, y_1,\ldots,\,y_m)$ 으로 흔히 표기한다. $L\in \mathcal{L}(\R^{n}\times \R^m,\, \R^m)$ 일 때 $L$ 를 $L_x\bf{x}=A(\bf{x},\,\bf{0}),\, L_y \bf{y} = A(\bf{0},\, \bf{y})$ 인 두 선형변환 $L_x,\,L_y$ 로 분리할 수 있다. 그렇다면
...
열린 $E\subset \R^{n}\times \R^m$ 에서 정의된 $F:E \to \R^m$ 이 $C^1$ 함수이고 어떤 $(\bf{a},\,\bf{b}) \in E$ 에서 $F(\bf{a},\bf{b})=\bf{0}$ 이라고 하자. $L=F'(\bf{a},\,\bf{b})$ 이고 $L_y$ 가 가역이면 어떤 $(\bf{a},\,\bf{b})$ 를 포함하는 열린집합 $U$ 와 $\bf{b}$ 를 포함하는 열린 집합 $W$ 가 존재하여

&emsp;($a$) 각각의 $\bf{y}\in W$ 에 대해 유일한 $\bf{x}\in \R^n$ 이 존재하여 $(\bf{x},\,\bf{y})\in U,\,F(\bf{x},\,\bf{y})=\bf{0}$ 이고,

&emsp;($b$) ($a$) 에 의해 $\bf{x}=G(\bf{y})$ 라 놓을 수 있는데, $G$ 는 $C^1$ 급 함수이며 $G(\bf{b})=\bf{a}$ 이며,
...
$W:=\{\bf{y}\in \R^n: (\bf{0},\,\bf{y}) \in V\}$ 라고 하자. $\bf{b}\in W$ 이며 $V$ 가 열린 집합이므로 $W$ 도 열린 집합이다(@exr-CM_for_implicit_theorem_rudin_1). $\bf{y}\in W$ 이면 $H(\bf{x},\bf{y})=(\bf{0},\bf{y})$ 인 $(\bf{x},\,\bf{y})\in U$ 가 존재하며 @eq-thm-CM_implicit_function_theorem_rudin_1 에 따라 $F(\bf{x},\bf{y})=\bf{0}$ 이다. $\overline{\bf{x}} \ne \bf{x}$ 에 대해 $(\overline{\bf{x}},\bf{y})\in U$ 이고 $F(\overline{\bf{x}},\bf{y})=\bf{0}$ 이라면
