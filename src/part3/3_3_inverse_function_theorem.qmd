...
이제 임의의 $\bf{y}\in \R^n$ 에 대해 $\phi:U \to \R^n$ 을 다음과 같이 정의하자.

$$
\phi(\bf{x}) := \bf{x} + A^{-1}(\bf{y}-F(\bf{x})).
$$
...
이다. @thm-CM_mean_value_theorem_for_vector_function 에 따라

$$
\bf{x}_1,\,\bf{x}_2 \in U \implies \|\phi_\bf{y}(\bf[])
$$
