...
($1$) 각각의 $\bf{y}\in W$ 에 대해 어떤 $\bf{x}\in \R^m$ 이 존재하여 $(\bf{x},\,\bf{y})\in U$ 이고 $F(\bf{x},\,\bf{y})=0$ 이고,

($2$) ($1$) 에 의해 $\bf{y}=G(\bf{x})$ 라 놓을 수 있는데, $G$ 는 $C^1$ 급 함수이며 $G(\bf{b})=\bf{a}$ 이며,

($3$) $G'(\bf{b}) = -(-A_x)^{-1}A_y$ 이다.
...
이다. $A_x$ 가 가역이므로 $\det (H'(\bf{a},\bf{b})) = \det (A_x) \ne 0$ 이고 따라서 $H'(\bf{a},\,\bf{b})$ 는 가역이다. 우선 $H'(\bf{a},\,\bf{b})$ 가 가역임을 보이자. $F(\bf{a},\,\bf{b})=\bf{0}$ 임을 고려하여
...
$W:=\{\bf{y}\in \R^n: (\bf{0},\,\bf{y}) \in V\}$ 라고 하자. $\bf{b}\in W$ 이며 $V$ 가 열린 집합이므로 $W$ 도 열린 집합이다(@exr-for_implicit_theorem_rudin_1).
