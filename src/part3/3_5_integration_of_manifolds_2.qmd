...
열린 집합 $A \subset \R^n$ 에 대해 어떤 $C^\infty$ 함수 $\phi: A \to \R$ 이 존재하여 $\phi$ 의 지지집합이 $A$ 에 포함된다.
...
라고 하자. 그렇다면 $K_{k+1}$ 은 옹골집합과 닫힌 집합의 교집합이므로 옹골집합이고 $K_{k+1}\subset U_{k+1}$ 이다. @exr-CM_spivak_1_22 에서 보였듯이 $K_{k+1} \subset \text{int}(D_{k+1})$ 이며  $D_{k+1}\subset U_{k+1}$ 을 만족하는 옹골집합 $D_{k+1}$ 이 존재한다. $K_1 = A - \bigcup_{i=2}^n U_i$ 을 통해 정할 수 있으며 따라서 모든 $D_i$ 를 정할 수 있다. 여기서 $A \subset \bigcup_{i=1}^n \text{int}(D_i)$ 이다. 

이제 $i=1,\ldots,\,n$ 에 대해 $D_i$ 를 지지집합으로 갖는 $C^\infty$ 함수 $\varphi_i$ 가 존재한다(@prp-CM_support_in_open_set) $\{D_1,\ldots,\,D_n\}$ 이 $A$ 를 덮으므로 $\bf{x}$ 가 $A$ 를 포함하는 어떤 열린 집합 $U$ 에 속했을 때 $\psi_1(\bf{x})+ \cdots + \psi_n(\bf{x}) > 0$ 이다. $\varphi_i:U \to \R$ 을 다음과 같이 정의한다.
...
열린 집합 $A\subset \R^n$ 를 생각하자. $\Phi=\langle \varphi_i\rangle$ 이 $A$ 에 포함되는 열린 덮개 $\mathcal{O}$ 에 종속된 $C^\infty$ 단위 분할 이며 $f$ 는 $A$ 에서 적분 가능하다고 하자. $\Psi = \langle \psi_j\rangle$ 역시 $A$ 에 포함되는 열린 덮개 $\mathcal{O}'$ 에 종속된 단위 분할일 때 $\sum_{\psi_j\in \Psi} \int_A \psi_j \cdot |f|$ 는 수렴하며 다음이 성립한다.
...
($1$) 어떤 옹골 집합 $K\subset A$ 을 제외하면 $\varphi_i \cdot f = 0$ 이며 이 $K$ 에서 유한개의 $\psi_j$ 만 $0$ 이 아니다. 따라서
...
($3$) $A$ 가 조르당 가측이고 $f$ 가 유계이면 $\sum_{\varphi_in \Phi} \int_A\varphi \cdot f = \int_A f$ 이다.
