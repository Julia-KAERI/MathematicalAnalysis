...
$A\subset \R^m,\,B\subset \R^n$ 이 닫힌 $n$-cell 이고 $f:A \times B \to \R$ 가 적분가능하다고 하자. $\bf{x}\in A$ 에 대해 $g_\bf{x}:B \to \R$ 을 $g_\bf{x}:=f(\bf{x},\bf{y})$ 로 정의하자. 그리고
...
$P_A \in \mathcal{P}(A),\, P_B \in \mathcal{P}(B)$ 라고 하자. $P_A\times P_B \in \mathcal{P}(A \times B)$ 이다. 따라서
