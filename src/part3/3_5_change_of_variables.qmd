...
이제 $i=1,\ldots,\,n$ 에 대해 $D_i$ 를 포함하며 $U_i$ 에 포함되는 콤팩트 지지집합을 갖는 $C^\infty$ 함수 $\psi_i$ 가 존재한다(@prp-CM_support_in_open_set) $\{D_1,\ldots,\,D_n\}$ 이 $A$ 를 덮으므로 $\bf{x}$ 가 $A$ 를 포함하는 어떤 열린 집합 $U$ 에 속했을 때 $\psi_1(\bf{x})+ \cdots + \psi_n(\bf{x}) > 0$ 이다. $\varphi_i:U \to \R$ 을 다음과 같이 정의한다.
...
::: {#lem-CM_partation_of_unity_and_integration_1}
...
정해진 $i$ 에 대한 $\supp \varphi_i$ 에서 $\Psi = \{\psi_j\}$ 가운데 유한개만 $0$ 이 아니다. 따라서
...
$A\subset \R^n$ 과 $f:A\to \R$ 이 유계라고 하자. $\Psi$ 가 $A$ 에 대한 열린 덮개 $\mathcal{O}$ 에 종속된 $C^\infty$ 단위 분할 일 때 $\sum_{\varphi\in \Phi} \int_A \varphi \cdot |f|$ 는 존재한다.
...
::: {#thm-CM_partation_of_unity_and_integration_1}
...
$\bf{I}$. 우선 $A$ 에 포함되는 열린 덮개 $\mathcal{O}$ 가 존재하며 각각의 $U\in \mathcal{O}$ 와 적분가능한 함수 $f$ 에 대해
