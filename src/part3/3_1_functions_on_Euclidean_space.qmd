...
@def-MA_countable 에서 $n$-cell 을 정의하였다. 이와 비슷하게 
...
$U^C$ 는 닫힌 집합이며 $C \cap U^C = \varnothing$ 이므로 @exr-CM_spivak_1_21 ($b$) 에서 보였듯이 모든 $y\in A,\, x\in B$ 에 대해 $\|y-x\|\ge d$ 인 $d>0$ 이 존재한다. 
...
($2$) $\|L\|<\infty$ 는 @exr-CM_spivak_1_10 에서 보였다. ($1$) 로부터 $\|L(\bf{x}-\bf{y})\|\le \|L\|\cdot \|\bf{x}-\bf{y}\|$ 이므로 고른 연속이다.
