...
두 벡터의 일반적인 내적은 $\langle \bf{x},\, \bf{y}\rangle$ 라고 쓰며 유클리드 내적은 $\bf{x}^T\bf{\cdot y}$ 라고 쓴다. 
...
함수 $\bf{f}:A\subset \R^n \to \R^m$ 은 $m$ 개의 **성분 함수(component function)** $f_i :\R^n \to \R$ $(i=1,\ldots,\,m)$ 를 가진다. 이 때 $\bf{f}(\bf{x}) = \begin{bmatrix} f_1(\bf{x}) & \cdots & f_m(\bf{x}) \end{bmatrix}^T= \begin{bmatrix} f_1(\bf{x}) \\ \vdots \\ f_m(\bf{x}) \end{bmatrix}$ 와 같이 열벡터로 표기한다.
...
($3$) $\bf{f} = \begin{bmatrix} f_1 & \cdots & f_m\end{bmatrix}^T$ 일 때 $f_i = \pi_i (\bf{f})$ 이다.
...
$\bf{f}:A \subset \R^n \to \R^m$ 이고 $\bf{a}\in A$ 일 때 다음이 성립함을 보여라.

$$
\displaystyle \lim_{\bf{x}\to \bf{a}} \bf{f}(\bf{x}) = \bf{b} \iff \forall i\in \{1,\ldots,\,m\}\, \lim_{\bf{x}\to \bf{a}} f_i(\bf{x}) = b_i
$$
...
$$
\|\bf{f}(\bf{x})-\bf{b}\| \le \sum_{i=1}^m |f_i(\bf{x})-b_i| < m \cdot \dfrac{\varepsilon}{m} = \varepsilon
$$
