...
$\left| \|\bf{x}\| -\|\bf{y}\|\right| \le \|\bf{x}-\bf{y}\|$ 임을 보여라.
...
선형 변환 $T:\R^n \to \R^n$ 이 $\|T(\bf{x})\|=\|\bf{x}\|$ 이면 **norm preserving** 이라고 하고 $T(\bf{x})
\bf{\cdot} T(\bf{y})= \bf{x \cdot y}$ 이면 **inner product preserving** 이라고 한다. 
...
($a$) $T$ 가 innerproduct preserving 이면 $\|T(\bf{x})\|^2  = T(\bf{x})\bf{\cdot}T(\bf{x}) = \bf{x\cdot x}=\|\bf{x}\|^2$ 이므로 norm preserving 이다. 이제 $T$ 가 norm-preservign 임을 가정하자. $T$ 가 선형결합이므로

$$
\begin{aligned}
\|T(\bf{x}-\bf{y})\|^2 &= \|T(\bf{x})-T(\bf{y})\|^2 = \|T(\bf{x})\|^2 - 2 T(\bf{x})\bf{\cdot}T(\bf{y})+ \|T(\bf{y})\|^2 \\
&=\|\bf{x}\|^2 - 2T(\bf{x})\bf{\cdot}T(\bf{y}) + \|\bf{y}\|^2 
\end{aligned}
$$
...
$$
\|T(\bf{x}-\bf{y})\|^2 = \|\bf{x}-\bf{y}\|^2 = \|\bf{x}\|^2 - 2\bf{x\cdot y}+ \|\bf{y}\|^2
$$

이다. 따라서 $T(\bf{x})\bf{\cdot}T(\bf{y})= \bf{x\cdot y}$ 이므로 $T$ 는 inner product preserving 이다.
...
선형변환 $T:\R^m \to \R^n$ 를 생각하자. 모든 $\boldsymbol{h}\in \R^m$ 에 대해 $\|T(\bf{h})\| \le M \|\bf{h}\|$ 인 $M$ 이 존재함을 보여라. 
...
$\bf{h} = (h_1,\ldots,\,h_m)$ 이며 $(T(\bf{h}))_i = \sum_{j=1}^m T_{ij}h_j$ 라 하자. 

$$
\left|\left(T(\bf{h})_i\right)\right| = \left| \sum_{j=1}^m T_{ij}h_j\right| \le \sum_{j=1}^m |T_{ij}|\cdot |h_j|
$$

이다. $M_0 = \max \{|T_{ij}| : 1 \le i \le n,\, 1 \le j \le m\}$ 이라고 하자. 

$$
\||T(\bf{h})\| \le \sum_{i=1}^n \left|\left(T(\bf{h})_i\right)\right| \le \sum_{i=1}^m \sum_{j=1}^n |T_{ij}|\cdot |h_j| \le \sum_{i, j}M_0 |h_j| \le \sum_{i, j}M_0 \|\bf{h}\|= mnM_0\|\bf{h}\|
$$
...
함수 $\bf{f}:A\subset \R^m \to \R^n$ 은 $n$ 개의 **성분 함수(component function)** $f_i :\R^m \to \R$ $(i=1,\ldots,\,n)$ 를 가진다. 이 때 $\bf{f}(\bf{x}) = (f_1(\bf{x}),\ldots,\,f_n(\bf{x}))$ 와 같이 쓴다. 
...
#### 성분 함수
항등함수 $\bf{\pi}:\R^n \to \R^n$ 은 $\bf{\pi}(\bf{x})=\bf{x}$ 이며 $\bf{\pi}$ 의 $i$ 번째 성분 함수를 $i$ 번째 **사영 함수(projection function)** 이라고 한다.
...
$f:A\subset \R^m \to \R$ 가 유계이며 $a\in A$ 일 때 정해진 $\delta>0$ 에 대해
...
$f:A\subset \R^m \to \R$ 이 유계이리 때 $f$ 가 연속인 것의 필요충분조건은 $o(f,\,\bf{a}) = 0$ 인 것이다.
...
닫힌 집합 $A\subset \R^m$ 에 대해 $f:A \to \R$ 이 유계일 때 임의의 $\varepsilon>0$ 에 대해 $\{\bf{x}\in A: o(f,\,\bf{x}) \ge \varepsilon\}$ 은 닫힌 집합이다.
...
$B=\{\bf{x}\in A: o(f,\,\bf{x}) \ge \varepsilon\}$ 라고 하고 $\R^m-B$ 가 열린 집합임을 보이자. $\bf{x}\in \R^m-B$ 이면 ($i$) $\bf{x}\notin A$ 이거나, ($ii$) $\bf{x}\in A$ 이며 $o(f,\,\bf{x}) <\varepsilon$ 이다. 

($i$) $\bf{x}\not\in A$ 이면 $A$ 가 닫힌 집합이므로 어떤 $\delta_x>0$ 에 대해 $N(\bf{x},\,\delta_x)\subset \R^m-A \subset \R^m-B$ 가 존재한다.

($ii$) $\bf{x}\in A$ 이며 $o(f,\,\bf{x})<\varepsilon$ 인 경우. 즉 어떤 $\delta>0$ 에 대해 $M(f,\,\bf{x},\,\delta)-m(f,\,\bf{x},\,\delta)<\varepsilon$ 이다. $\bf{y}\in N(\bf{x},\,\delta)$ 라면 어떤 $N(\bf{y},\, \delta_1)\subset N(\bf{x},\, \delta)$ 인 $\delta_1>0$ 이 존재한다. 즉 $N(\bf{x},\,\delta)\subset \R^m-B$ 이다. 

즉 $R^m-B$ 는 열린 집합이므로 $B$ 는 닫힌 집합이다.
...
$\bf{f}:A \subset \R^m \to \R^n$ 이고 $\bf{a}\in A$ 일 때 다음이 성립함을 보여라.

$$
\displaystyle \lim_{\bf{x}\to \bf{a}} \bf{f}(\bf{x}) = \bf{b} \iff \forall i\in \{1,\ldots,\,n\}\, \lim_{\bf{x}\to \bf{a}} f_i(\bf{x}) = b_i
$$
...
($\implies$) 임의의 $\varepsilon>0$ 에 대해 $0<\|\bf{x}-\bf{a}\|<\delta \implies \|\bf{f}(\bf{x})-\bf{b}\|<\varepsilon$ 인 $\delta>0$ 이 존재한다. $|f_i(\bf{x})-b_i|\le\|f(\bf{x})-\bf{b}\|$ 이므로 성립한다. 


($\impliedby$) 임의의 $\varepsilon>0$ 에 대해 $0<\|\bf{x}-\bf{a}\|<\delta \implies |f_i(\bf{x})-b_i|<\varepsilon/n$ 인 $\delta>0$ 이 존재한다. 그렇다면.

$$
\|\bf{f}(\bf{x})-\bf{b}\| \le \sum_{i=1}^n |f_i(\bf{x})-b_i| < n \cdot \dfrac{\varepsilon}{n} = \varepsilon
$$
...
$f:A\subset \R^m \to \R^n$ 이 $\bf{a}$ 에서 연속일 필요충분조건은 각각의 $f_i$ 가 $\bf{a}$ 에서 연속인 것임을 보여라.
...
::: {.solution}
$\bf{b}$ 를 $\bf{f}(\bf{a})$ 로 고치고 @exr-part3_spivak_1_23 를 적용한다. 
:::
