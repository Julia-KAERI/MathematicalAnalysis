...
상수 $t$ 와 좌표공간의 점 $A$ 에 대하여 $|tA|=|t||A|$ 임을 보여라.
...
3차원 좌표공간에서 선분 $S$ 를 $yz$-평면, $zx$-평면, $xy$-평면에 정사영한 것의 길이가 각각 5, 6, 7 이라면 선분 $S$ 의 길이는 얼마인가?
...
삼차원 좌표공간에서 길이가 $l$ 인 선분을 $yz$-평면, $zx$-평면, $xy$-평면에 정사영한 것의 길이를 각각 $l_{yz},\, l_{zx},\, l_{xy}$ 라고 두면
...
$$
\begin{aligned}
\left| {l_{yz}}^2-{l_{zx}}^2 \right|&=\left|(y_1-y_2)^2-(x_1-x_2)^2\right| \\
&\le |(y_1-y_2)^2+(x_1-x_2)^2 |={l_{xy}^2}\,,\\
{l_{xy}}^2 &=|(y_1-y_2)^2+(x_1-x_2)^2| \\
&\le |(y_1-y_2)^2+(x_1-x_2)^2+2(z_1-z_2)^2| \\
&={l_{yz}}^2+{l_{zx}}^2\,.
\end{aligned}
$$
...
집합 $\{1,\,2,\ldots,\, n\}$ 에서 정의된 함수 전체의 집합은 $\mathbb{R}^n$ 과 일대일 대응이 됨을 보이라.
...
$$
f(x) =a_0+a_1x+a_2x^2+a_3x^3,\, g(x) = b_0+b_1x+b_2x^2+b_3x^3
$$ 
...
$$
\begin{aligned}
T(f+g) &= (a_0+b_0,\, a_1+b_1,\, a_2+b_2,\,a_3+b_3)\,,\\[0.3em]
...
::: {#exr-CAL_4_1_11} 
 $n$-공간의 점 $A=(a_1,\ldots,\,a_n)$ 에 대하여 $\overline{a}:=\dfrac{1}{n}(a_1+\cdots + a_n)$ 이라 하자. 이 때 점 $A$ 와 $\overline{A}=(\overline{a},\ldots,\,\overline{a})$ 사이의 거리는
...
좌표평면에서 극좌표가 $(r_1,\, \theta_1),\, (r_2,\,\theta_2)$ 로 주어진 두 점 사이의 거리를 구하라.
...
극좌표계로 주어진 다음 곡선의 방정식을 직교좌표계의 벙정식으로 바꾸고, 곡선의 개형을 그리라. 단 $a>0$ 이다.
...
![problem_4_2_2](figures/problem_4_2_2.jpg)
...
![problem_4_2_3](figures/problem_4_2_3.jpg)
...
![problem_4_2_4](figures/problem_4_2_4.jpg)
...
::: {#exr-CAL_4_2_5}
 ...
 주어진 실수 $\theta_0$ 와 양수 $r_0$ 에 대하여 극좌표계에서 식
...
직교좌표계에서 중심이 $R(\cos \theta_0,\, \sin \theta_0)$ 이고 반지름의 길이가 $r_0$ 인 원의 방정식은 극좌표계로는
...
$$
\begin{aligned}
(x-R\cos \theta_0)+&(y-R\sin \theta_0)^2-{r_0}^2 = 0 \\[0.3em]
&\implies (r\cos \theta-R\cos \theta_0)^2 + (r \sin \theta -R \sin \theta_0)^2-{r_0}^2=0 \\[0.3em]
&\implies r^2+R^2 -2rR (\cos \theta \cos \theta_0 + \sin \theta \sin \theta_0)-{r_0}^2=0\\[0.3em]
...
다음 구면좌표계로 표현된 점들을 원기둥좌표계와 구면좌표계로 표시하여라.
...
다음 직교좌표계로 표현된 식들을 원기둥좌표계와 구면좌표계로 표시하여라.
...
직교좌표계에서 원뿔 $x^2+y^2 \le z^2$ 와 공 $x^2+y^2+z^2 \le z$ 의 공통부분으로 이루어진 영역을
...
집합 $U$ 의 원 $x,\,y$ 에 대하여 순서쌍 $(x,\,y)$ 는 집합 $\{\{x\},\, \{x,\,y\}\}$ 로 정의한다. 이 때 $(x,\,y)=(x',\,y')$ 일 필요충분조건은 $x=x',\, y=y'$ 임을 보이라.
...
![problem_4_x_2_butterfly](figures/problem_4_x_2_butterfly.jpg)
...
![problem_4_x_2](figures/problem_4_x_2.jpg)
...
구간 $[0,\,1]$ 에서 연속인 함수 전체의 집합을 $\mathcal{C}^0 [0,\,1]$ 이라고 두자. 이때 두 함수 $f,\,g\in \mathcal{C}^0[0,\,1]$ 에 대하여
...
$$
\|f\|\|g\|=\left(\int_0^1 |f(x)|^2\,dx\right)^2  \left(\int_0^1 |g(x)|^2\,dx\right)^2 \ge \int_0^1 f(x)g(x)\, dx 
$$ {#eq-exr_CAL_4_S03_1}

을 얻었다.

이제 $\|f\|+\|g\| \ge \|f+g\|$ 가 성립함을 보이자. @eq-exr_CAL_4_S03_1 을 이용하여

$$
\begin{aligned}
(\|f\|+\|g\|)^2 &= \|f\|^2+2\|f\|\|g\|+\|g\|^2 \ge \|f\|^2+\|g\|^2 +2\int_0^1 f(x)g(x)\,dx \\[0.3em]
&= \int_0^1 (f(x)^2+g(x)^2+2f(x)g(x))dx =\|f+g\|^2
\end{aligned}
$$
