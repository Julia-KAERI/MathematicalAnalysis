...
**1.** 다음 함수에서 $D_1D_2 f= D_2 D_1 f$ 가 성립하는가를 조사하라.
...
**2.** 함수 $f(x,\,y)=xy^2+ye^{-x}\sin (x-y)$ 의 모든 이계편도함수를 구하라.
...
**4.** 이급함수 $f(x,\,y)$ 는 정의역의 각 점에서 임의의 벡터 $\mathbf{v}$ 에 대하여 $\mathbf{v}$ 방향 이계미분계수와 $(-\mathbf{v})$ 방향 이계미분계수가 같은가?
...
**5.** 원점 근방에서 정의된 삼급함수 $f(x,\,y)$ 가 원점에서 $\mathbf{v}$-방향 이계미분계수가 $1$ 이라고 하자. 이 때 $2\mathbf{v}$-방향 이계미분계수는 얼마인가? 또 $f(x,\,y)$ 가 원점에서 $\mathbf{v}$-방향 삼계미분계수가 $1$ 이면 $2\mathbf{v}$-방향 삼계미분계수는 얼마인가?
...
**6.** 곡선 $(x_1(t),\ldots,\, x_n(t))$ 와 함수 $f(x_1,\ldots,\,x_n)$ 에 대하여,
...
**9.** $ \left(\dfrac{\partial}{\partial x} + x \dfrac{\partial}{\partial y}\right)^2$ 과 $\left(\dfrac{\partial}{\partial x}\right)^2 + 2x \dfrac{\partial^2}{\partial x \partial y}+   x^2\left(\dfrac{\partial}{\partial y}\right)^2$ 는 같지 않음을 보이라.
...
**10.** 함수 $f(x,\,y) = \ln (1+xy)$ 에서 ${D_{\mathbf{v}}}^2 f,\, {D_{\mathbf{v}}}^3f $ 를 구하라. 그리고 점 $(1,\,1)$ 에서 $\mathbf{v}=(1,\,2)$ 방향으로 볼록인지 또는 오목인지 조사하라.
...
**12.** $n$-공간의 한 열린집합 $U$ 에서 정의된 무한급함수 전체의 집합을 $C^\infty(U)$ 라고 두자. 그러면 임의의 두 벡터 $\mathbf{v},\,\mathbf{w}$ 에 대하여 방향미분작용소
$$
...
**13.** 함수 $f(x_1,\ldots,\, x_n)$ 이 $k$ 급 함수이면, 임의의 벡터 $\mathbf{v} = \sum_{i=1}^n v_i \mathbf{e}_i$ 에 대하여
...
$n$-공간의 열린 집합 $U$ 에서 정의된 $k$-번 미분가능한 함수 $f$ 가 주어졌다고 하자. 이 때 $U$ 의 한 점 $P$ 와 벡터 $\mathbf{v}$ 에 대하여 선분 $[P,\, P+\mathbf{v}] : = \{P+t\mathbf{v}: 0\le t \le 1\}$ 이 $U$에 포함된다고 하자. 그러면
$$
...
**2.** 원점에서 함수 $f(x,\,y)=\cos x \sin y$ 의 3차 근사 다항식을 구하라.
...
**4.**  점 $(x,\,y)$ 가 원점에 가까이 갈 때, $\sin (xy)/x$ 의 극한값은 존재하는가? 존재하면 그 값을 구하라. (분모가 영이 아닌 영역에서만 조사한다.) 또 다음 함수들에 대하여는 어떠한가?
...
**5.** 함수 $x^3-2xy^4+(x-1)^2y^10$ 의 테일러급수에서 제 7차항은 무엇인가?
...
**6.** 함수 $f(x,\,y,\,z)$ 가 $x,\,y,\,z$ 의 다항식이면, $f$ 는 자기 자신의 테일러 급수와 같음을 보이라.
...
**7.** 함수 $x^2y+xy^2+1$ 을 $(x-1)$ 과 $(y-1)$ 의 멱급수로 표현하라.
...
**8.** 이급함수 $f(x,\,y)$ 가 임의의 $(x,\,y)$ 와 실수 $t$ 에 대하여
...
**9.** 기준점에서의 값을 $1$ 로 두었을 때 다음의 일차 근사값을 구하고, 또 오차를 생각하라.
...
**10.** 함수 $f$ 와 $g$ 가 무한급함수이면 정의역의 점 $P$ 와 벡터 $\mathbf{v}$ 및 자연수 $k$ 에 대하여
...
$$
\begin{aligned}
{D_{\mathbf{v}}}^n(fg) = \sum_{k=0}^n \dfrac{n!}{k! (n-k)!} {D_{\mathbf{v}}}^k f (P,\,\mathbf{v})\,{D_\mathbf{v}}^{n-k} g(P,\,\mathbf{v})
\end{aligned}
$$
...
$$
\begin{aligned}
T_k (fg)(P,\,\mathbf{v}) &= \sum_{n=0}^k \dfrac{{D_{\mathbf{v}}}^n (fg)(P)}{n!}  \\
...
&= \sum_{n=0}^{2k} \sum_{m=0}^n \dfrac{1}{m! (n-m)!} {D_{\mathbf{v}}}^m f(P)\cdot {D_{\mathbf{v}}}^{n-m} g(P) \\
&= \sum_{n=0}^k \sum_{m=0}^n \dfrac{1}{m! (n-m)!} {D_{\mathbf{v}}}^m f(P)\cdot {D_{\mathbf{v}}}^{n-m} g(P) \\
&\quad + \sum_{n=k+1}^{2k}\sum_{m=0}^n \dfrac{1}{m! (n-m)!} {D_{\mathbf{v}}}^m f(P)\cdot {D_{\mathbf{v}}}^{n-m} g(P) \\
&= T_{k}(fg)(P,\,\mathbf{v})+  \sum_{n=k+1}^{2k}\sum_{m=0}^n \dfrac{1}{m! (n-m)!} {D_{\mathbf{v}}}^m f(P)\cdot {D_{\mathbf{v}}}^{n-m} g(P)
\end{aligned}
$$

$C^\infty$ 급 함수 $f,\,g$ 에 대해 ${D_{\mathbf{v}}}^m f(P) = O(|\mathbf{v}|^m)$, ${D_{\mathbf{v}}}^{n-m} g(P) = O(|\mathbf{v}|^{n-m})$ 이므로,

$$
T_k f(P,\,\mathbf{v})\cdot T_k g(P,\,\mathbf{v}) -T_{k}(fg)(P,\,\mathbf{v}) = o(|\mathbf{v}|^n)
$$
...
**2.** (**최소자승법**) 좌표평면의 점 $(x_1,\,y_1),\ldots,\, (x_n,\,y_n)$ 에 대하여
...
**3.** 함수 $f(x,\,y)=3x^4-4x^2y+y^2=(3x^2-y)(x^2-y)$ 에 대하여,
...
**4.** (**Steiner** 문제) 평면의  $\triangle ABC$ 에서 꼭지각의 크기가 모두 $120^\circ$ 미만이라고 하자. 이때 함수
...
**2.** 함수 $f(x,\,y)=(x^2+y^2)\cos (x+2y)$ 는 원점이 극소점임을 보이라.
...
**5.** 이차형식 $q(x,\,y)=ax^2+2bxy+cy^2$​ 의 등위선이 원점을 지나지 않을 때 이 등위선은
(1) $ac-b^2>0$​ 이면, 타원
(2) $ac-b^2<0$​ 이면, 쌍곡선
(3) $ac-b^2=0$​ 이면, 직선
임을 보여라.
...
**6.** $n$ 공간의 점 $P$ 근방에서 정의된 이급함수 $f$ 와 벡터 $\mathbf{v}=v_1\mathbf{e}_1 + \cdots + v_n \mathbf{e}_n$ 에 대하여
...
**1.** 원판 $x^2+y^2\le 1$ 에서 정의된 함수 $f(x,\,y)=x^2+y^2-x-y+1$ 의 최대값과 최소값을 구하라.
...
**2.** 반지름의 길이가 1 인 구면에 내접하는 원기둥 중에서 표면적을 최대로 하는 것의 표면적을 구하라. 원기둥은 바닥과 뚜껑이 없는 것으로 한다.
...
**4.** (**케플러의 문제**) 대각선의 길이가 일정한 직원기둥중에서 부피가 최대인것을 구하라. 또 주어진 공(sphere) 속에 들어있는 직육면체 중에서 그 부피가 최대인 것을 구하라.
...
**5.** CBS 부등식을 라그랑즈 승수법을 이용하여 증명하라.
...
**6.** $0<p<q$ 이고 $a_1\ge 0,\ldots,\, a_n \ge 0$ 이면, 부등식
...
**7.** 포물면 $z=x^2+y^2$ 과 점 $(1,\,1,\,0)$ 사이의 최단거리를 구하라.
...
**8.** 곡면 $z=xy$ 의 점 중에서 점 $(0,\,0,\,5)$ 에 가장 가까운 점을 모두 구하라.
...
**9.** 점 $(1,\,2,\,0)$ 에서 원뿔 $z^2=x^2+y^2$ 까지의 거리를 구하라.
...
**14.** $n$ 차 대칭행렬 $Q=(q_{ij})$ 와 $X=\begin{pmatrix} x_1 \\ \vdots \\ x_n\end{pmatrix}$ 에 대하여, 이차형식은
$$
...
**15.** 영 이상의 실수 $x,\,y,\,z$ 가  $x+y+z=1$ 을 만족 시킬 때 $xy+yz+zx-2xyz$ 의 최대값과 최소값을 구하라.
...
**17.** 좌표평면의 원점에서의 곡선 $x^6+y^6=1$ 의 한 점에 이르는 거리의 최대값과 최소값을 구하라.
...
**1.** $n$-공간에서 주어진 벡터 $\mathbf{v}$ 와 무한급 미분가능함수 $f,\,g$ 에 대하여,
...
**4.** ($\textbf{H\"older}$ 부등식 ) 양수 $p,\,q$ 가 $\dfrac{1}{p}+\dfrac{1}{q}=1$ 을 만족시킬 때, 벡터
...
**5.** 함수 $L(x,\,y,\,z)$ 에 대하여 공간 $\mathcal{F}=\{ f\in \mathcal{C}^1[a,\,b] \mid f(a)=y_a,\, f(b)=y_b\}$  에서 정의된 범함수
...
**6.** 최단강하선 문제에서 $f$ 가 직선이거나 원호이면 걸리는 시간은 얼마인가.
