...
**1.** 곡선 $(2t^2,\, 1-t,\,3+t^2)$ 과 평면 $3x-14y+z=10$ 의 교점을 구하라.
...
$\bf{f}(t)=(e^t,\, e^{2t},\, 1-e^{-t})$, $\bf{g}(t) = (1-t,\, \cos t,\, \sin t)$ 라 하자. $\bf{f}(0)=(1,\, 1,\, 0),\, \bf{g}(0)=(1,\, 1,\, 0)$ 이므로 만난다. $t=0$ 일 때의 점선의 방향벡터는,
...
**4.** 원점을 지나지 않는 미분가능한 곡선 $X(t)\in \mathbb{R}^3$ 에 대하여,
...
**5.** 극좌표계에서 $r=r(\theta)$ 로 주어진 평면곡선은 직교좌표계로는
...
**6.** 탁자 위에서 원판이 일정한 각속도 $3 \operatorname{rad/sec}$ 로 돌고 있다. (이때 회전의 중심이 원판의 중심이다.) 원판의 중심에서 파리 한마리가 원판의 둘레의 한 점을 향하여 (원판에 대하여) $2 \operatorname{cm/sec}$ 로 움직인다고 하자. 원점에서 출발 후 $4$ 초가 되었을 때 탁자에 대한 파리의 속력을 구하라.
...
**7.** 평면 위에서 점 $P$ 의 운동이 $(\cos 2t,\, \sin 2t)$ 로 주어지고, 또 점 $Q$ 의 운동이 $2(\cos t,\,\sin t)$ 로 주어졌다고 하자. 점 $P$ 에서 점 $Q$ 를 바라보았을 때 $Q$ 의 운동을 $X(t)$ 로 둘 때, 이를 구하고 도시하라.
...
**8.** (**Bézier 의 이차곡선**) 공간 상의 주어진 서로 다른 세 점 $P_0,\,P_\ast,\, P_1$ 에 대하여 곡선
...
**2.** 정규곡선 $X(t)$ 에서 속력 $v(t)$ 와 가속력 $a(t)$ 를 각각
...
**3.** $X(t)=r(t) (\cos \theta(t),\, \sin \theta(t))$ 일 때,
...
$$
\begin{aligned}
f'(t) &= \underbrace{X'(t) \cdot X'(t)\times X''(t)}_0 + \underbrace{X(t) \cdot X''(t)\times X''(t)}_0 + X(t) \cdot X'(t) \times X'''(t) \\
...
**5.** 곡선 $X(t)=(\sin t,\, \cos t,\, t)$ 의 $t=\dfrac{\pi}{2}$ 인 점에서 접촉평면의 식을 구하라.
...
**3.** (**피타고라스의 짝** 과 원의 매개화) 자연수 $p,\,q,\,r$ 이
...
**4.** 곡선 $X:[a,\,b]\to \mathbb{R}^n$ 의 에너지는
...
**5.** 곡선의 에너지는 매개화를 달리하였을 때 변화하는지를 살펴보라.
...
**6.** 삼차원 공간 속의 곡선 $X(t)$ 에 대하여 $t=t_0$ 에서
