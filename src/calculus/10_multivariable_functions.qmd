...
**2.** 다음 함수의 그래프를 종이와 연필을 이용하여 그리라. 또 컴퓨터 프로그램을 이용하여 그리라.
...
**6.** 좌표공간의 부분집합에서 정의된 연속함수가 최대값을 가지지 않는 경우는 어떠한 경우인가?
...
**2.** 다음 함수의 $\mathbf{v}$ 방향 미분계수를 구하라.
...
**3.** $n$ 공간의 열린 집합 $U$ 에서 정의된 편미분가능한 함수 $f,\,g$ 에 대하여
...
**4.** $n$-공간에서 항등사상 $X= (x_1,\ldots,\,x_n):\mathbb{R}^n \to \mathbb{R}^n$ 과
...
**6.** $n$-공간에서 $r=|X|$ 라 할 때 점 $X$ 에서 다음 함수들의 기울기 벡터를 구하라.
...
**8.** 좌표평면의 극좌표계 $(r,\,\theta)$ 에서 $r=\sqrt{x^2+y^2}$ 이므로,
...
**2.** 좌표평면에서 정의된 미분가능함수 $f(x,\,y)$ 가 원점에서, $(1,\,2)$-방향 미분계수가 $1$ 이고 $(2,\,3)$ 방향 미분계수가 $2$ 라고 하자. 원점에서의 이 함수의 $(3,\,5)$ 방향 미분계수는 얼마인가? 또 기울기 벡터 $\nabla f(O)$ 를 구하라.
...
**4.** 직사각형 $(a,\,b)\times (c,\,d)\subset \mathbb{R}^2$ 에서 정의된 미분가능함수 $f(x,\,y),\, g(x,\,y)$ 가 항등적으로
...
**5.** 좌표평면의 원점 근방에서 정의된 함수 $f(x,\,y)$ 에 대하여 $D_1 f(O)=1,\, D_2 f(O)=-2$ 라 하자. 이 대 함수값의 변화가 거의 없는 방향은 어느 방향인가?
...
**1.** 좌표평면의 표준직교좌표계 $(x,\,y)$ 와 극좌표계 $(r,\,\theta)$ 에 대하여
...
**2.** (**오일러 공식**) 주어진 자연수 $k$ 에 대하여 함수 $f(x_1,\ldots,\,x_n)$ 이
...
**3.** 미분가능함수 $f(\mathbf{x})$ 가 임의의 실수 $t$ 와 임의의 $\mathbf{x}\in\mathbb{R}^n$ 에 대하여 $f(t\mathbf{x})=tf(\mathbf{x})$  이면
...
**4.** 미분가능함수 $f$ 에 대하여 $z=f\left(\dfrac{x-y}{y}\right)$ 로 둘 때,
...
**6.** 연속함수 $g(t)$ 와 일급함수 $a(x,\,y),\, b(x,\,y)$ 에 대하여, 함수
...
**9.** $x^3+x^2y+xy^2+y^3=1$ 일 때 $\dfrac{dy}{dx}(1)$ 을 구하라.
...
**10.** 독도 근방의 바다에서 (경도, 위도)- 좌표를 $(x,\,y)$ 라 하자. 어느날 이 근방의 해수 온도가 $f(x,\,y)\, ({}^\circ \text{C})$ 라고 하자. 이때 바다 지점 $(x_0,\, y_0)$ 에서 동쪽으로 움직일 경우 순간 온도 변화율이 $+1\, (^\circ \text{C/rad})$ 이고, 북쪽으로 움직일 경우 순간 온두 변화율이 $-1\, (^\circ \text{C/rad})$ 라고 하자. 만약 독도 근방을
...
**12.** $w=f(x-y,\, y-x)$ 일 때, $\dfrac{dw}{dx}+\dfrac{dw}{dy}=0$ 임을 보이라.
...
**13.** $w=x^3 f\left(\dfrac{y}{x},\, \dfrac{z}{x}\right)$ 일 때, $x\dfrac{\partial w}{\partial x}+ y \dfrac{\partial w}{\partial y}+z\dfrac{\partial w}{\partial z}=3w$ 임을 보이라.
...
**14.** $g(x,\,y)=f(x+y,\, x-y)$ 이고 $x+y=u,\, x-y=v$ 라 하자. 이때 다음이 성립함을 보이라.
...
**15.** 상수 $\theta$ 에 대하여 $x=u\cos \theta -v \sin \theta,\, y=u \sin \theta+v \cos \theta$ 라 하자. $f(x,\,y)=g(u,\,v)$ 일 때,
...
**16.** 미분가능한 일변수 함수 $g(r)$ 의 도함수가 영이 되지 않는다고 하자. 이때 함수
...
**17.** 미분가능한 함수 $f(X)$ 의 기울기 벡터가 $X$ 와 나란하면
...
**3.** 다음 식으로 주어진 곡면의 점 $P$ 에서 접평면의 방정식을 구하라.
...
주어진 $\{\mathbf{v}_1,\ldots, \mathbf{v}_n\}$  가 일차독립이므로 $n$ 공간의 기저이다. 따라서, 모든 표준단위벡터 $\{\mathbf{e}_i\}$ 를 $\{\mathbf{v}_j\}$ 의 선형결합으로, 즉 $\displaystyle \mathbf{e}_i=\sum_{j=1}^n a_{ji}\mathbf{v}_j$ 와 같이 쓸 수 있다. 그렇다면,
...
**1.** 실수체 $\mathbb{R}$ 에서 유리수체 $\mathbb{Q}$ 의 내부, 외부 및 경계를 구하라.
...
**3.** $n$ 공간의 부분집합 $A$ 가 열린 집합일 필요충분조건은 그 여집합 $A^C=\mathbb{R}^n-A$ 가 닫힌 집합인 것임을 보여라.
...
**4.** $n$ 공간의 부분집합 $A$ 가 닫힌 집합일 필요충분조건은 $A$ 에 속하지 않은 모든 점이 $A$ 의 외점인 것을 보이라.
...
**5.** 두 열린 집합의 합집합과 교집합은 모두 열린집합임을 보여라.
...
**6.**  두 닫힌 집합의 합집합과 교집합은 모두 열린집합임을 보여라.
...
**6.** $n$-공간의 정규직교기저  $\mathbf{v}_1,\ldots,\,\mathbf{v}_n$ 과 미분가능 함수 $f(x_1,\ldots,\,x_n)$ 에 대하여
...
**9.** 좌표공간의 한 닫힌 집합 $A$ 에서 정의된 연속함수 $f:A \to \mathbb{R}$ 가
...
**10.** 좌표공간의 표준 좌표계 $(x,\,y,\,z)$ 에서 얻은 벡터장 $(\partial_x,\,\partial_y,\,\partial_z)$ 와 구면좌표계 $(\rho,\,\varphi,\,\theta)$ 에서 얻은 벡터장 $(\partial_{\rho},\,\partial_{\varphi},\,\partial_{\theta})$​ 에 대하여 다음을 보이라.
