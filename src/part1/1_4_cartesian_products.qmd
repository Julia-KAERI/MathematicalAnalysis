...
를 $X$ 의 원소에 대한 **$\bf{n}$-tuple** 이라 한다. $n$-tuple $\bf{x}$ 에 대해 $\bf{x}(i)$ 를 $x_i$ 라고 쓰며 $\bf{x}$ 의 $i$ 번째 좌표라고 한다. $n$-tuple 자체를
...
집합족 $\{A_1,\ldots,A_m\}$ 과 인덱스 집합 $J_m=\{1,\ldots,\,m\}$ 에 대해 $X=A_1 \cup \cdots \cup A_m$ 이라 하자. $\bf{x}:J_m \to X$ 이 $\forall i\in J_m,\, x_i \in A_i$ 인 집합을 **데카르트 곱(Cartesian Product)** 이라 하며 다음과 같이 표기한다.
...
집합 $X$ 에 대해 $\mathbb{Z}_+$ 를 정의역으로, $X$ 를 공역으로 정의된 함수 $\bf{x}:\mathbb{Z}_+ \to X$ 를 $\bf{\omega}$**-tuple** 혹은 $X$ 에서의 **수열(sequence)** 이라고 하며 $\bf{x}(i)$ 를 $x_i$ 라고 쓴다. $\omega$-tuple 을 $\langle x_i \rangle$ 이라고 흔히 쓴다. $n$-tuple 과 유사하게
...
($5$) $\bf{a}_j\in A^\omega$ $(j=1,\,2,\ldots, n)$ 에 대해 $a_{ij} = (\bf{a}_j)_i$ $(i=1,\,2,\ldots)$ 라 하자. $k\in \mathbb{Z}_+$ 에 대해 $p,\, q$ 는 각각 $k$ 를 $n$ 으로 나누었을 때의 몫과 나머지라 하자.

$$
m((\bf{a}_1,\ldots,\, \bf{a}_n))_k = (\bf{a}_{q+1})_{p+1} 
$$
...
&emsp;($a$) $\{\bf{x} \mid x_i \in \mathbb{Z}\}$. 

&emsp;($b$) $\{\bf{x} \mid \forall i,\, x_i \ge i\}$.

&emsp;($c$) $\{\bf{x} \mid \forall i\ge 100,\, x_i \in \mathbb{Z} \}$.

&emsp;($d$) $\{\bf{x} \mid x_2= x_3 \}$.
