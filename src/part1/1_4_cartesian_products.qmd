...
::: {#def-union_and_intersection_for_collection}
...
$m\in \mathbb{Z}_+$ 라 하자. 주어진 집합 $X$ 에 대해 함수 

$$
\boldsymbol{x} : \{1,\ldots,m\} \to X
$$

를 $X$ 의 원소에 대한 **$\boldsymbol{m}$-tuple** 이라 한다. $m$-tuple $\boldsymbol{x}$ 에 대해 $\boldsymbol{x}(i)$ 를 $x_i$ 라고 쓰며 $\boldsymbol{x}$ 의 $i$ 번째 좌표라고 한다. $m$-tuple 자체를
...
::: {.callout-note appearance="simple" icon="false"}
::: {#def-omega_tuple_and_sequence}

#### $\boldsymbol{\omega}$-tuple 과 수열 (sequence)

집합 $X$ 에 대해 $\mathbb{Z}_+$ 를 정의역으로, $X$ 를 공역으로 정의된 함수 $\boldsymbol{x}:\mathbb{Z}_+ \to X$ 를 $\boldsymbol{\omega}$**-tuple** 혹은 $X$ 에서의 **수열** 이라고 하며 $\boldsymbol{x}(i)$ 를 $x_i$ 라고 쓴다. $\omega$-tuple 을 $\langle x_i \rangle$ 이라고 흔히 쓴다. $m$-tuple 과 유사하게
...
($a$) $\mathbb{Z}^\omega \subset \mathbb{R}^\omega$ (see @exr-munkres_1_5_3).
