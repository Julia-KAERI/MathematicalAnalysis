...
::: {#def-rule_of_assignment}

#### 할당 규칙 (rule of assignment)

$R\subset C \times D$ 에 대해 $C$ 의 각각의 원소가 $R$ 의 첫번째 좌표로 최대 한번만 나올 때 $R$ 을 $A \times B$ 의 할당 규칙이라 한다.
...
할당 규칙 $R$ 에 대해 정의역 $A$ 와 상을 포함하는 집합 $B$ 를 생각하자. 이 때 $B$ 를 공역(codomain) 이라 한다. 이 할당규칙을 $A$ 에서 $B$ 로의 함수라고 하며 할당규칙 $R$ 에 대한 함수를 $f$ 라고 하면 $f:A \to B$ 라 표현한다. $(a,\,b)\in R$ 이면 $f(a) = b$ 라고 쓴다.
...
::: {#def-injection_surjection_bijection}
...
@def-dictionary_order 에서의 사전 순서는 $A \times B$ 에서의 전순서 관계 이다.
...
::: {#def-least_upper_bound_property}
