...
$f:A \to B$ 를 생각하자. 어떤 함수 $g: B \to A,\, h : B \to A$ 가 존재하여 모든 $a\in A$ 에 대해 $g(f(a)) =a$ 이며 모든 $b\in B$ 에 대해 $f(h(b))=b$ 이면 $f$ 는 전단사이며 $g=h$ 이다. 
...
각각의 $b\in B$ 에 대해 $f(x)=b$ 를 만족하는 $x\in A$ 는 단 하나 뿐이며 이를 $a$ 라 하자. $g(b) = a$ 이며 $f(h(b))=b$ 이므로 $h(b) = a$ 이다. 즉 $g=h$ 이다. $\square$
:::
...
$R$ 이 $A$ 에 서의 관계일 때 다음 조건을 모두 만족하면 $R$ 을 **동치 관계 (equivalence relation)** 라고 한다.
...
전순서 집합 $A$ 의 부분집합 $A_0$ 을 생각하자. 아래를 만족하는 $a\in A_0$ 가 존재한다면 $a$ 를 $A_0$ 의 **최대값** 이라고 한다.

$$
\forall x\in A_0 \implies x\le a.
$$ {#eq-FA_definition_of_maximum}

반대로 아래를 만족하는 $b\in A_0$ 가 존재한다면 $b$ 를 $A_0$ 의 **최소값** 이라고 한다.

$$
\forall x\in A_0 \implies x\ge b.
$$ {#eq-FA_definition_of_maximum} 
...
전순서 집합 $A$ 의 부분집합 $A_0$ 에 대해 어떤 $a\in A$ 가 존재하여 $x\in A_0 \implies x\le a$ 이면 $A_0$ 를 **위로 유계(bounded above)** 라 하고 $a$ 를 $A_0$ 의 **상계(upper bound)** 라 한다. 어떤 $a'\in A$ 가 존재하여 $x\in A_0 \implies a' \le x$ 라 하면 $A_0$ 를 **아래로 유계(bounded below)** 라 하고 $a'$ 을 $A_0$ 의 **하계(lower bound)**라 한다. 또한 $A_0$ 가 위로 유계이면서 동시에 아래로 유계이면 $A_0$ 를 **유계 (bounded)** 라고 한다.
