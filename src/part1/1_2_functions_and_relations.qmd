...
$f:A \to B$ 가 전단사 함수 일 때 @prp-inverse_function 에서의 $g=h$ 를 $f$ 에 대한 **역함수(inverse)** 라 하고 $f^{-1}$ 이라 쓴다. 
...
::: {#prp-properties_of_inverse_image}
...
::: {#prp-properties_of_equivalence_class}
...
::: {#thm-equivalence_relation_and_partition}
...
Reflectivity 에 의해 모든 $a$ 의 집합은 어떤 동치류에 포함된다. 따라서 동치류의 합집합은 $A$ 이다. @prp-properties_of_equivalence_class 에 의해 두 동치류는 같거나 서로소이므로 동치류의 집합족은 $A$ 를 분할한다. 
...
::: {#prp-dictionary_order_is_order_relation}
...
::: {#thm-least_upper_bound_property_greatest_lower_bound_property}
다음 두 명제는 동치이다.
