...
::: {#thm-strong_induction_principle}
...
::: {#thm-archmedian_ordering_property}
...
::: {#cor-archimedean_ordering_property}
...
($1$) @thm-least_upper_bound_property_greatest_lower_bound_property 에서 최소상계 성질을 갖는 집합은 최대 하계 성질을 가짐을 보였다.

($2$) $A=\{1/n: n\in \mathbb{Z}+\}$ 라 하자. @exr-munkres_1_4_2 의 ($9$) 에 의해 $0$ 은 $A$ 의 하계이다. ($1$) 에 의해 $A$ 는 최대 하계를 가진다. $a=\inf A$ 라 하자. $a>0$ 이라면 @thm-archmedian_ordering_property 에 의해 $m>1/a$ 인 $m\in \mathbb{Z}_+$ 이 존재하며 $1/m<a,\, 1/m \in A$ 이므로 $a$ 가 최대 하계라는 가정에 모순된다. $a<0$ 이라면 모든 $n \in \mathbb{Z}_+$ 에 대해 $1/a<1/(2a)<n$ 이므로 역시 $a$ 가 최대 하계라는 가정에 모순된다. 따라서 $\inf A=0$ 이다.
...
($1$) $A\subset \mathbb{Z},\, A\ne \varnothing$ 이며 위로 유계라고 하자. $A$ 가 모두 양의 정수의 집합이라면 @exr-munkres_1_4_4 ($1$) 에 의해 최대값을 가진다. $A$ 가 양의 정수를 포함하는 집합이라면 $A \cap \mathbb{Z}_+$ 는 최대값을 가지며 이 값이 $A$ 의 최대값이다. $A$ 가 $0$ 과 음의 정수로 이루어진 집합이라면 $0$ 이 최대값이다. $A$ 가 음의 정수만으로 이루어진 집합이라고 하자. $A'=\{-a\mid a\in A\}$ 는 양의 정수의 집합이며 @thm-well_ordering_property 에 의해 최소값 $m$ 을 가진다. 즉 모든 $n\in A'$ 에 재해 $m\le n$ 이며 $-n \le -m$ 이므로 $-m$ 이 $A$ 의 최대값이다.

($2$) $A_x =\{m\in \mathbb{Z}\mid m<x\}$ 라 하자. 우선 $A_x \ne \varnothing$ 임을 보이자. $x>0$ 이라면 $0\in A_x$ 이므로 $A_x\ne \varnothing$ 이다. $x=0$ 이라면 $-1\in A_x$ 이므로 $A_x \ne \varnothing$ 이다. $x<0$ 이라면 @thm-archmedian_ordering_property 에 의해 $p>-x$ 인 $m\in \mathbb{Z}_+$ 가 존재하며 $-p<x$ 이므로 $-p\in A_x$ 이다. 즉 $A_x \ne \varnothing$ 이다. 
...
($4$) 이것은 중요하므로 아래 @prp-Q_is_dense 로 정리하도록 한다.
...
$y=0$ 이면 @cor-archimedean_ordering_property 으로 보일 수 있다. $0<y<x$ 인 경우 아르키메데스 법칙(@cor-archimedian_law) 에 의해
