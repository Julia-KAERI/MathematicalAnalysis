...
[뒤에서 나오겠지만 이런 성질을 조밀성 이라 한다.]{.aside}
...
양의 정수 $n$ 에 대해 $1$ 보다 크거나 같고 $n$ 보다 작은 자연수의 집합을 **section** 이라 하며 $S_n$ 으로 쓴다. 
...
아래의 대수적 관계를 @def-field 의 공리를 이용하여 증명하라.
...
아래의 부등식 규칙을 @def-field 와 @def-ordered_field  의 공리를 이용하여 증명하라.
...
($3$) @def-rational_irrational 에 의해 $a\in \mathbb{Q}$ 는 어떤 정수 $p,\,q$ 에 대해 $a=p/q$ 이다. 만약 둘 다 짝수이면 $p=2m,\, q=2n$ 이므로 $a=m/n$ 이다. 이 과정을 $m,\,n$ 둘 중 하나가 짝수가 아닐 때까지 반복 할 수 있다.
