...
$A\subset \mathbb{R}$ 이 다음 성질을 가지면 **귀납적(be inductive)** 이라고 한다. 
...
아래와 같이 정의된 $\mathbb{Q}$ 의 원소를 **유리수 (rational number)** 라 한다. 어떤 실수가 유리수가 아니면 **무리수 (irrational number)** 라 한다.
$$
