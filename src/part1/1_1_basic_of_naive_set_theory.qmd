...
집합의 원소가 모두 집합인 집합을 **집합족 (colleciton/family)**이라 한다. 특히 집합 $A$ 의 부분집합의 집합은 집합족으로 $A$ 의 **멱집합 (power set)** 이라고 한다. 
...
집합 $A,\,B$ 에 대해 $a\in A,\, b\in B$ 일 때 $(a,\,b) = \{ \{a\}, \, \{a, b\}\}$ 를 **순서쌍 (ordered pair)** 이라 한다. 이 때 $(a,\,b)$ 에서 $a$ 를 첫번째 **좌표 (coordinate)**, $b$ 를 두번째 좌표라고 한다.
