...
집합 $A$ 가 공집합이거나 $A$ 에서 section(@def-part1_section) 로 가는 전단사 함수가 존재할 때 이 $A$ 를 **유한집합**이라 한다. $A$ 가 공집합일 경우 **cardinality 0** 라 하며, $\{1,\ldots,\,n\}$ 과 전단사 관계가 존재할경우 **cardinality n** 이라 한다. 
...
$n\in C$ 임을 가정한다. $f:A \to \{1,\ldots,n+1\}$ 이 전단사 함수라고 하자. $A$ 의 진부분집합 $B$ 에 대해 $a_0\in B$ 와 $a_1\in A\backslash B$ 를 선택한다. @lem-part1_finite_set1 에 의해 전단사 함수 $g:A\backslash \{a_0\} \to \{1,\ldots,n\}$ 가 존재한다. 이로부터 가정에 의해 아래의 결론을 내릴 수 있다.

&emsp; ($1$) $B\backslash\{a_0\} \mapsto \{1,\ldots,n\}$ 인 전단사 함수는 존재하지 않으며, 

&emsp; ($2$) 어떤 $m<n$ 에 대해 전단사 함수 $h:B\backslash\{a_0\} \to \{1,\ldots,\,m\}$ 이 존재한다. 

@lem-part1_finite_set1 에 의해 $B \mapsto \{1,\ldots,n+1\}$ 인 전단사 함수는 존재하지 않는다. 또한 $B\backslash \{a_0\} \ne \varnothing$ 일 경우 $B\mapsto \{1,\ldots,\,m+1\}$ 인 전단사 함수가 존재한다. 즉 $C=\mathbb{Z}_+$ 이다. $\square$
...
$f:A \to B$ 가 전단사 함수라 하자. $g:A \to \{1,\ldots,\,n\}$ 이 전단사함수라면 $g\circ f^{-1}$ 는 $B \mapsto \{1,\ldots,n\}$ 전단사 함수인데 이는 @thm-part1_finite_set2 에 위배된다. $\square$
...
::: {#cor-part1_Z_plus_is_not_finite}
...
$f: \mathbb{Z}_+ \to \mathbb{Z}_+ \backslash \{1\}$ 을 $f(n) =n+1$ 로 정의하면 전단사 함수이다. 그런데 $\mathbb{Z}_+\backslash \{1\} \subsetneq \mathbb{Z}_+$ 이므로 @cor-part1_finite_set2 에 의해 $\mathbb{Z}_+$ 는 유한집합이 아니다. $\square$

:::

</br><div class="border" style="background-color:#FFF0F5 ;padding:5px;">

::: {#cor-part1_cardinality_of_finit_set}
유한집합의 cardinality 는 유일하게 정해진다.
:::
</div></br>

::: {.proof}
$A$ 와 $\{1,\ldots,\,n\}$ 사이의 전단사 함수 $f$ 와 $A$ 와 $\{1,\ldots,m\}$ 사이의 전단사 함수 $g$ 가 존재한다고 하자. $g \circ f^{-1}$ 은 $\{1,\ldots, n\}$ 에서 $\{1,\ldots,\,m\}$ 으로의 전단사 함수이어야 하는데, $m\ne n$ 이라면 둘중 하나는 나머지의 진부분집합이므로 @cor-part1_finite_set2 에 의해 전단사 함수가 존재하지 않는다. 즉 $m=n$ 이므로 유한집합의 cardinaility 는 유일하게 정해진다. $\square$

:::

</br><div class="border" style="background-color:#FFF0F5 ;padding:5px;">

::: {#cor-part1_cardinality_of_subset}
...
$A$ 의 cardinality 가 $n$ 이라면 @thm-part1_finite_set2 에 의해 $B$ 의 cardinality 는 $n$ 보다 작다. $\square$

:::

</br><div class="border" style="background-color:#FFF0F5 ;padding:5px;">

::: {#cor-part1_properties_of_finite_set}
...
::: {#prp-part1_finite_cartesian_product_of_finite_sets}
...
$A$ 가 유한집합이라면 어떤 양의 정수 $n$ 에 대해 전단사 함수 $f: A \to \{1,\ldots,\,n\}$ 가 존재한다. $B \subset A$ 이면 @cor-part1_cardinality_of_subset 에 의해 $B$ 는 유한집합이어야 하므로 가정에 위배된다. 

:::

</br><div class="border" style="background-color:#F0FFFF  ;padding:5px;">

::: {#exr-part1_munkres_1_6_3}
...
::: {#exr-part1_munkres_1_6_4}
#### Munkres 1.6.4

$A$ 는 공집합이 아닌 유한 전순서 집합(@def-part1_order_relation) 이라 하자.

&emsp; ($1$) $A$ 에 최대값이 존재함을 보이시오.

&emsp; ($2$) $A$ 는 어떤 section 과 같은 순서형(@def-part1_ordered_pair) 을 가짐을 보이시오.
