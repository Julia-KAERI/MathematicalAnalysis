...
::: {#exm-part2_case_of_derivative_without_continuity_1}
...
::: {#exm-part2_case_of_derivative_without_continuity_2}
...
@exm-part2_case_of_derivative_without_continuity_1 과 @exm-part2_case_of_derivative_without_continuity_2 에서 보았듯이 $n$-차 도함수가 연속일 수도 있고 아닐 수도 있다. $n$ 차 도함수가 연속인 것은 매우 중요한 성질이므로 여기에 특별한 이름을 붙인다.
...
### 도함수의 연속성 {#sec-part2_the_continuity_of_derivatives}

@exm-part2_case_of_derivative_without_continuity_2 에서 보았듯이 함수 $f$ 에 대한 도함수 $f'(x)$ 가 모든 값에서 존재하지만 특정 값에서 불연속일 수 있다. 하지만 도함수의 불연속은 항상 2종 불연속인데 여기서는 그것을 보인다.

</br>

<div class="border" style="background-color:#FFF0F5 ;padding:5px;">

::: {#thm-part2_continuity_of_derivative}
...
$g(t) = f(t)-\lambda t$ 라고 하자. $g'(t) = f'(t)-\lambda$ 이므로 $g'(a)<0,\, g'(b)>0$ 이다. 따라서 어떤 $t_1\in (a,\,b)$ 에서 $g(t_1)>g(a)$ 이며, 역시 어떤 $t_2\in (a,\,b)$ 에서 $g(t_2)<g(b)$ 이다. @thm-part2_extreme_value_theorem 에 따라 어떤 $x\in (a,\,b)$ 에서 $g$ 는 최소값을 가진다. 이 때 $g'(x)=0$ 이며 따라서 $f'(x) = \lambda$ 이다. $\square$
...
::: {#cor-part2_continuity_of_derivative}
...
이다 $f(\alpha) = \dfrac{f^{(0)}(\alpha)}{0!}(\beta-\alpha)^0$ 이므로 @eq-exr-part2_rudin_5_18 이 성립한다. 
...
($1$) @eq-part2_rudin_5_19 을 아래와 같이 바꿀 수 있다.
...
\end{aligned}
$$ {#eq-part2_rudin_5_19_1}

이다. 따라서 $\displaystyle \lim_{n \to \infty} D_n = f'(0)$ 이다.


($2$) $0<\alpha_n<\beta_n<1$ 이면 @eq-part2_rudin_5_19_1 식은 다음과 같이 변한다.
...
($a$) $g(t) := f(t)-t$ 라고 하면 $g'(t)=f'(t)-1$ 이므로 $g'(t)\ne 0$ 이다. $t_1,\,t_2\in \R$ 에 대해 $g'(t_1)g'(t_2)<0$ 이면 @thm-part2_continuity_of_derivative 에 따라 $t_1,\,t_2$ 사이에 $g'(t)=0$ 인 $t$ 가 존재하므로 모든 $t$ 에 대해 $g'(t)>0$ 이거나 $g'(t)<0$ 이다. 즉 단조증가이거나 단조감소이므로 $g(t)=0$ 는 하나 이상의 해를 가질 수 없다. 
...
::: {#exr-part2_rudin_5_24}

#### Rudin 5.24

@exr-part2_rudin_5_22 의 ($d$) 에서 설명된 과정은 $(0,\,\infty) \mapsto (0,\,\infty)$ 함수에도 적용 될 수 있다. 고정된 $\alpha>1$ 에 대해 

$$
f(x) = \dfrac{1}{2} \left(x+\dfrac{\alpha}{x}\right),\qquad g(x) = \dfrac{\alpha+x}{1+x}
$$

라고 하자. $f,\,g$ 모두 $\sqrt{\alpha}$ 가 $(0,\,\infty)$ 에서의 유일한 고정점이다. $f,\,g$ 의 성질로부터 @exr-part2_rudin_3_16 의 수렴이 @exr-part2_rudin_3_17 의 수렴보다 빠른지 설명하라. 
...
![Plots of $f(x)$ and $g(x)$ with $\alpha =3$](images/pp1.png){#fig-part2_rudin_5_24 width=400}
...
이다. 즉 $\xi<x_{n+1}<x_n$ 이다. $\langle x_n \rangle$ 은 유계인 단조수열이므로 수렴한다. $\lim_{n \to \infty} x_n = t$ 라 하면 @eq-part2_rudin_5_25_1 로부터 $t=t-f(t)/f'(t)$ 를 만족해야 하는데 이 값은 $\xi$ 뿐이다. 
...
$f(\xi)=0$ 이며 @eq-part2_rudin_5_25_1 를 이용하면
...
($f$) $f(x) = x^{1/3}$ 의 해는 $x=0$ 뿐이다. @eq-part2_rudin_5_25_1 로 부터 $x_{n+1}=x_n - 3x_n=-2x_n$ 이 되어 $\langle x_n \rangle$ 이 발산한다. 
...
이다. @exr-part2_rudin_5_26 에 따라 $f([a,\,b])= \{0\}$ 이므로 $f_1=f_2$ 이다. 즉 위의 미분방정식을 만족하는 해는 많아야 하나 존재한다.
...
::: {#exr-part2_rudin_5_28}

#### Rudin 5.28, 미분방정식의 해의 유일성 2

@exr-part2_rudin_5_27 과 유사한 유일성 정리를 미분방정식 시스템에 대해 증명한다. 아래와 같은 미분방정식들이 존재한다고 하자.
