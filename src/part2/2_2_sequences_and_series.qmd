...
($4$) 자연수 $n$ 에 대해 $N(p,\, 1/n) \cap E \ne \varnothing$ 이므로 $N(p,\, 1/n) \cap E$ 에서 하나를 잡아 $a_n$ 이라 하자. 그렇다면 $\langle a_n\rangle\to  p$ 이다. $\square$
...
이제 $\langle a_n \rangle$ 이 무한집합일 경우에 대해 생각하기로 하자. @prp-infinite_subset_in_compact_set_has_limit_point_in_compact_set 의해 $\langle a_n \rangle$ 은 극한점을 가지며, 따라서 $k\in \mathbb{Z}_+$ 에 대해 $N(a,\, 1/k)\cap \langle a_n \rangle$ 은 무한개의 원소를 갖는다. $p_1=1$ 로 하고  $N(a,\, 1/k) \cap \left(\langle a_n \rangle \backslash \{a_1,\ldots,\, a_{p_k}\} \right)$  가운데 $a_m$ 을 골라 $p_{k+1}= m$ 이라고 하자. 그렇다면 $\langle a_{p_k}\rangle$ 은 $a$ 로 수렴하는 부분수열이다. $\square$
...
유계인 수열을 포함하는 $n$-cell 은 옹골집합이므로 @prp-properties_of_subsequence 에 의해 수렴하는 부분수열을 가진다. $\square$
...
($4$) @prp-properties_of_subsequence 과 ($3$) 에 의해 성립한다. $\square$ 
...
($1$) 코시 판정법(@thm-convergence_of_series_1) 에 의해 $\sum_k b_k$ 이 수렴하면 임의의 $\varepsilon>0$ 에 대해 $N \in \mathbb{Z}_+$ 이 존재하여 $m\ge n \ge N$ 이면 $\displaystyle \sum_{k=n}^m b_k < \varepsilon$ 이며, 따라서 $\displaystyle\sum_{k=n}^m a_k <\varepsilon$ 이다.

($2$) $\sum_k a_k$ 이 수렴한다면 코시 판정법(@thm-convergence_of_series_1) 에 의해 $\sum_k b_k$ 도 수렴해야 하는데 이는 조건에 위배된다. 따라서 $\sum a_k$ 은 발산한다. $\square$
...
::: {.proof}
($1$) $\alpha<1$ 이라 하자. $\alpha< \beta < 1$ 인 임의의 $\beta$ 를 생각하자. @prp-properties_of_limsup 에 의해 $k\ge N \implies \sqrt[k]{|a_k|}<\beta$ 인 자연수 $N$ 이 존재한다. 즉 $k \ge N \implies |a_n|< \beta^n$ 이므로 $\sum_k a_k$ 는 절대수렴한다.

($2$) $\alpha>1$ 이라 하자. @prp-properties_of_limsup 에 의해 $1<\beta<\alpha$ 일 때 무한히 많은 $k$ 에 대해 $|\alpha_k|> \beta^k > 1$ 이다. 따라서 $\sum_k a_k$ 는 발산한다.

($3$) @exm-sequences 에서 $\displaystyle \lim_{k \to \infty} \sqrt[k]{k}=1$ 임을 보였다. 따라서 $\displaystyle \lim_{k \to \infty} \dfrac{1}{\sqrt[k]{k}} = \lim_{k \to \infty} \dfrac{1}{\sqrt[k]{k^2}}=1$ 이다. 그런데 $\alpha=1$ 인 두 수열 $\sum 1/k,\, \sum 1/k^2$ 은 각각 발산하고 수렴한다. (@prp-convergence_of_1_over_n_p) $\square$
...
($1$) $\displaystyle \limsup_{k \to \infty} \left|\dfrac{a_{k+1}}{a_k}\right| =\alpha$ 라고 하자. 임의의 $\alpha <\beta< 1$ 인 $\beta$ 에 대해 $k \ge N_0 \implies \left|\dfrac{a_{k+1}}{a_k}\right|<\beta$ 인 $N_0 \in \mathbb{Z}_+$ 이 존재한다(@prp-properties_of_limsup). 임의의 $m\in \mathbb{Z}_+$ 에 대해 $|a_{N_0 + m}|< \beta^m |a_{N_0}|$ 이므로 $\sum_k a_k$ 는 수렴한다. 

($2$) $k \ge N \implies |a_{k+1}|\ge |a_k|$ 이면 $\displaystyle \lim_{k \to \infty}a_k \ne 0$ 이므로 $\sum_k a_k$ 은 발산한다 (@cor-convergence_of_series_2). $\square$
:::
...
이므로(@prp-properties_of_limsup), $m\in \mathbb{Z}_+$ 에 대해 $a_{N+m} \ge a_N(\alpha - \varepsilon)^m$ 이며, 
...
::: {#exm-convergence_of_some_series}
...
임을 가정하자. $r= \dfrac{c-(a+b)}{2}$ 라 하면 $r>0$ 이다. @prp-properties_of_limsup 에 의해 $\varepsilon < \dfrac{r}{2}$ 에 대해 어떤 $N\in \mathbb{Z}_+$ 이 존재하여,
