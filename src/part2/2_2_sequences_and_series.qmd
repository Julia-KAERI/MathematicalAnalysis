...
&emsp;($4$) $\mathbb{R}^k$ 위에서의 코시수열은 수렴한다.
...
($4$) ($2$) 에 의해 코시수열은 유계이므로 코시수열을 포함하는 $n$-cell 을 잡을 수 있다. $n$-cell 은 옹골집합이므로 @prp-properties_of_subsequence 에 의해 수렴하는 부분수열을 갖는다. ($3$) 에 의해 이 코시수열은 수렴한다. $\square$ 
...
&emsp;($1$) 임의의 $\varepsilon>0$ 에 대해 $n \ge N \implies a_n < a+\varepsilon$ 을 만족하는 $N\in \mathbb{N}$ 이 존재한다.
...
&emsp;($2$) 임의의 $\varepsilon>0$ 에 대해 $b + \varepsilon < b_n$ 을 만족하는 $n$ 이 무한히 많이 존재한다.
...
&emsp;($1$) $\alpha<1$ 이면 $\sum a_n$ 은 수렴하며,

&emsp;($2$) $\alpha > 1$ 이면 $\sum a_n$ 은 발산하고,
...
&emsp;($1$) $\displaystyle \limsup_{n \to \infty} \left|\dfrac{a_{n+1}}{a_n}\right| <1$ 이면 수렴한다.

&emsp;($2$) 어떤 $N\in \mathbb{N}$ 에 대해 $n\ge N \implies \left|\dfrac{a_{n+1}}{a_n}\right| \ge 1$ 이면 발산한다.
...
다음 급수의 수렴과 발산을 근판정법과 비율판정법으로 확인 할 수 있다.
...
($1$) 의 경우를 보자. $a_{2n-1} = \dfrac{1}{2^n},\, a_{2n}= \dfrac{1}{3^n}$ 이므로,
...
($2$) 의 경우는 $a_{2n-1}= \dfrac{1}{2^{2n-1}}, a_{2n}= \dfrac{1}{2^{2n-2}}$ 이므로
...
($1$) 우선 $(m+1)^2-(m)^2 = 2m+1$ 이므로 인접하는 $0$ 보다 큰 두 자연수의 제곱의 차이는 2보다 크다. 따라서 $\sqrt{n-1}$ 과 $\sqrt{n+1}$ 이 동시에 제곱수일 수는 없다. 

($2$) 제곱수가 아닌 자연수 $n$ 에 대해 $\sqrt{n}$ 이 유리수라고 가정하면 서로소인 어떤 자연수 $p,\,q$ 에 대해 $\sqrt{n}=p/q$ 이며 $q^2 n = p^2$ 인데 이로부터 $q\mid p$ 라는 결론이 나오므로 모순이다. 따라서 제곱수가 아닌 자연수의 제곱근은 무리수이며 따라서 $\sqrt{n-1}+ \sqrt{n+1}$ 은 자연수 + 무리수 이거나, 무리수 + 무리수 이다. 자연수 + 무리수 인 경우는 무리수이므로, 무리수 + 무리수 인 경우만 살펴보면 된다.

($3$) 서로소인 자연수 $p,\,q$ 에 대해 $\sqrt{n-1}+ \sqrt{n+1} = \dfrac{p}{q}$ 라 하자. 그렇다면 $\sqrt{n^2-1}=\dfrac{p^2}{2q^2} - \dfrac{2n+1}{2} \in \mathbb{Q}$ 이다. 그런데 ($1$) 에서 보였듯이 $n^2-1$ 은 완전제곱수가 아니며 따라서 $2$ 에 의해 무리수이다. 즉 $\sqrt{n-1}+ \sqrt{n+1}$ 은 무리수이다.
:::
...
(1) 만일 $|a|<1$ 이면 $\lim_n a^n = 0$ 이고, $|a|>1$ 이면 $\langle a^n\rangle$ 은 발산한다.

(2) 임의의 양수 $a>0$ 에 대하여 $\lim_n a^{1/n}=1$ 이다.
