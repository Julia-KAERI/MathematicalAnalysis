...
$$
\forall \varepsilon>0,\, \exists N\in \mathbb{Z}_+, \text{ s.t. } n \ge N \implies d(a_n,\, a)<\varepsilon
$$

일 때 수열 $\langle a_n \rangle$ 은 $a$ 로 수렴한다고 하며 $\displaystyle \lim_{n \to \infty} a_n = a$ 혹은 $\langle a_n\rangle \to a$ 라고 쓴다. $\langle a_n \rangle$  이 수렴하지 않으면 발산한다고 한다. 
...
($1$) $\implies$ 는 정의에 의해 자명하다. $a$ 의 임의의 근방 $N_X(a,\,\varepsilon)$ 에 대해 유한개 성분을 제외한 $a_n$ 의 모든 성분이 포함된다고 하자. 유한개의 포함되지 않는 수열 성분 가운데 인덱스가 최대인 것을 $N$ 이라고 하면 $n\ge N \implies a_n \in N_X(a,\,\varepsilon)$ 이므로 $\langle a_n \rangle \to a$ 이다. 


($2$) $a_1,\,a_2$ 로 수렴한다고 하자. 고정된 $\varepsilon>0$ 에 대해 $n\ge N_1 \implies d(a_n,\,a_1)<\varepsilon$ 이며 $n \ge N_2 \implies d(a_n,\, a_2)<\varepsilon$ 이라 하자. $n \ge \max (N_1,\, N_2)$ 이면 $d(a_1,\,a_2)  \le d(a_n,\,a_1)+d(a_n,\, a_2) < \varepsilon$ 이다. 임의의 $\varepsilon>0$ 에 대해 성립해야 하므로 $a_1=a_2$ 이다.
...
이므로 $\langle a_n\rangle$ 은 유계이다. $\square$
...
&emsp;($1$) $\displaystyle \lim_{n \to \infty}(a_n+b_n) = a+b$.

&emsp;($2$) $c\in \mathbb{R}$ 에 대해 $\displaystyle \lim_{n \to \infty} (ca_n)= ca$.

&emsp;($3$) $\displaystyle \lim_{n \to \infty} a_nb_n = ab$.

&emsp;($4$) $a\ne 0$ 일 때 $\displaystyle \lim_{n \to \infty} \dfrac{1}{a_n}= \dfrac{1}{a}$.

&emsp;($5$) 어떤 $N \in \mathbb{N}$ 에 대해 $n\ge N \implies a_n \ge 0$ 이면 $a\ge 0$ 이다.
...
($1$) 고정된 $\varepsilon>0$ 에 대해 $n\ge N_1 \implies |a_n - a|<\varepsilon/2$, $n \ge N_2 \implies |b_n- b|<\varepsilon/2$ 인 $N_1,\,N_2$ 가 존재한다. $n \ge \max(N_1,\,N_2)$ 이면
...
($2$) $c=0$ 이면 자명하다. $c\ne 0$ 일 때 고정된 $\varepsilon>0$ 에 대해 $n\ge N \implies |a_n - a|<\varepsilon/|c|$ 인 $N$ 이 존재한다. 따라서 $n \ge N \implies |ca_n - ca| = |c||a_n -c| < \varepsilon$ 이다.

($3$) 주어진 $\varepsilon>0$ 에 대해 $n\ge N_1 \implies |a_n-a|<\sqrt{\varepsilon}$, $n \ge N_2 \implies |b_n -b|<\sqrt{\varepsilon}$ 인 $N_1,\, N_2$ 가 존재하며 $N = \max (N_1,\, N_2)$ 라 하자. 

$$
a_nb_n - ab = (a_n-a)(b_n-b) + a(b_n-b)+ b(a_n-a)
$$

인데 $n \ge N \implies |(a_n-a)(b_n -b)|<\varepsilon$ 이며 ($1$) 과 ($2$) 에 의해 $a(b_n-b)$ 와 $b(a_n-a)$ 는 $0$ 으로 수렴하므로 $\displaystyle \lim_{n \to \infty} a_nb_n = ab$ 이다.
