...
$\mathbb{Z}_+$ 가 정의역으로 정의된 함수를 **수열(sequence)** 라고 하며 $a$ 가 수열일 때 $a(n)$ 을 $a_n$ 이라고 표기한다. 수열 전체는 $\langle a_n\rangle$ 이라 표기한다.
...
::: {#prp-MA_countable_subset}

$A \sim \mathbb{Z}_+$ 의 무한 부분집합 $B$ 는 $B \sim \mathbb{Z}_+$ 이다.

:::
</div>
::: {.proof}

$A\sim J$ 이므로 $x:J \to A$ 인 수열 $\langle x_n \rangle$ 을 생각 할 수 있다. $n_1$ 을 $x_n \in B$ 가운데 가장 작은 $n$ 값으로 하자(@thm-FA_well_ordering_property 에 의해 항상 가장 작은 인덱스를 정할 수 있다). $n_2$ 는 그 다음 작은 값... 이렇게 계속 하면 $n_1,\ldots,\,n_k,\ldots$ 를 정할 수 있다. 이 때 $f(k)= x_{n_k}$ 라고 하면 $\langle f_n \rangle = B$ 이므로 $B \sim J$ 이다. $\square$
:::
...
::: {#thm-MA_countable_union_of_countable_sets}
...
::: {#prp-MA_countable_cartesian_product}
...
는 가산이며 $B_{n-1}\times A$ 는 @thm-MA_countable_union_of_countable_sets 에 의해 가산이다. 따라서 $B_n$ 은 가산이다. $\square$
...
::: {#thm-MA_Cantor_diagonal_process}
...
::: {#cor-MA_real_numbers_are_uncaoutable}
...
집합 $X$ 에 아래와 같은 성질을 갖는 이항함수 $d_X:X\times X \mapsto \mathbb{R}$ 이 정의될 때 이 함수를 **거리 함수(metric)** 라고 하고 $X$ 를 **거리공간(metric space)** 라고 한다. 
...
이 때 $(a,\,b),\, [a,\,b]$ 를 각각 **열린구간 (open interval)**, **닫힌구간 (closed interval)** 이라고 한다.
...
::: {#def-MA_Euclidean_space}

양의 정수 $n$ 에 대해 유클리드 내적이 정의된 벡터공간 $\mathbb{R}^n$ 를 $n$ 차원 **유클리드 공간(Euclidean space)** 이라고 한다. $\bf{x} \in \mathbb{R}^n$ 는 $\bf{x}=(x_1,\ldots,\,x_n)$ 로 표현 할 수 있으며 $x_j$ 를 $\bf{x}$ 의 $j$ 번째 **좌표(coordinate)** 라고 한다. $\bf{x},\, \bf{y}\in \mathbb{R}^n$ 에 대한 유클리드 내적은 $\bf{x\cdot y}$ 로 쓰며

$$
\bf{x \cdot y} := \sum_{j=1}^n x_j y_j
$$

로 정의된다. $\bf{x}$ 의 노름(norm) $\|\bf{x}\|$ 는 $\|\bf{x}\|:=(\bf{x \cdot x})^{1/2}$ 로 정의된다. 앞으로 특별한 언급이 없다면 $\mathbb{R}^n$ 는 $n$ 차원 유클리드 공간을 의미한다.
...
::: {#prp-MA_norm_of_Euclidean_space}

$\bf{x},\, \bf{y},\,\bf{z}\in \mathbb{R}^n$ 과 $a\in \mathbb{R}$ 에 대해 다음이 성립한다.

&emsp;($1$) $\|\bf{x}\| \ge 0$ 이며 $\|\bf{x}\|=0 \iff \bf{x}=\bf{0}$ 이다.

&emsp;($2$) $\|a\bf{x}\| = |a| \|\bf{x}\|$.

&emsp;($3$) $\|\bf{x}-\bf{y} \| = \| \bf{y}-\bf{x} \|$.

&emsp;($4$) $\|\bf{x \cdot y}\| \le \|\bf{x}\| \|\bf{y}\|$.

&emsp;($5$) $\| \bf{x}+\bf{y}\| \le \|\bf{x}\| + \|\bf{y}\|$.

&emsp;($6$) $\| \bf{x}-\bf{y} \| \le \|\bf{x}-\bf{z} \| + \|\bf{z}-\bf{y}\|$.
...
$$
\|\bf{x}+\bf{y}\|^2 = \|\bf{x}\|^2 +2 \bf{x \cdot y} + \|\bf{y}\|^2 \le (\|\bf{x}\|+\|\bf{y})^2
$$

임을 보일 수 있다. 이로부터 $\| \bf{x}+\bf{y}\| \le \|\bf{x}\| + \|\bf{y}\|$ 이다.

($6$) ($5$) 에서 $\bf{x}$ 를 $\bf{x}-\bf{z}$ 로, $\bf{y}$ 를 $\bf{z}-\bf{y}$ 로 바꾼다. $\square$
:::
...
### 열린집합과 닫힌집합 {#sec-MA_open_set_and_closed_set}
...
$$
[a_1,\,b_1]\times \cdots \times [a_n,\,b_n] := \{(x_1,\ldots,\,x_k)\in \mathbb{R}^n : a_i \le x_i \le b_i,\, i=1,\ldots,n\}
$$
...
$$
\begin{aligned}
N_X(p,\, \varepsilon) &:= \{x\in X : d_X(p,\,x) <\varepsilon\}, \\
\overline{N}_X(p,\,\varepsilon) & := \{ x \in X : d_X(p,\,x) \le \varepsilon \}.
\end{aligned}
$$

$N_X(p,\,\varepsilon)$ 을 $p$ 를 중심으로 반지름 $\varepsilon$ 인 $X$ 에서의 **열린 근방 (open neighborhood)** 이라고 하고, $\overline{N}_X(p,\,\varepsilon)$ 을 $p$ 를 중심으로 반지름 $\varepsilon$ 인 $X$ 에서의 **닫힌 근방 (closed neighborhood)** 이라고 한다. 특별한 언급 없이 근방이라고 할 때는 열린 근방을 의미한다.
...
$$
\begin{aligned}
|(1-t)x+ty-p| &= |(1-t)(x-p) + t(y-p)| \\[0.3em]
& \le (1-t)|x-p| + t|y-p| \\[0.3em]
...
### 거리공간에서의 위상 {#sec-MA_topology_in_metric_space}

::: {.callout-note icon="false"}

#### **위상학적 용어들**

::: {#def-MA_topologycal_terms}
...
3. 모든 $E$ 의 극한점의 집합을 $E'$ 라고 표기한다. $E' \subset E$ 일 때 $E$ 가 **닫혀있다(closed)** 라고 하며 닫혀있는 집합을 **닫힌 집합(closed set)** 이라고 한다. $\overline{E}=E \cup E'$ 으로 정의하며 $E$ 의 **폐포(closure)** 라고 한다.
...
7. $\exists M>0,\;\forall p,\,q \in E, \, d_X(p,\,q)<M$ 일 때 $E$ 를 유계라고 한다.
...
($1\implies 2$) $p\in E'$ 의 근방이 유한개의 $E$ 의 원소를 포함한다고 가정하자. 포함된 $E$ 의 원소중 $p$ 와의 거리가 최소인 $p$ 가 아닌 원소를 $q$ 라 하자. $N(p,\, d(p,\,q)/2)\cup E$ 는 $p \in E$ 이면 $\{p\}$ 이고 $p \notin E$ 이면 $\varnothing$ 이므로 $p$ 가 극한점이라는 가정에 모순된다. 따라서 무한개의 $E$ 의 원소를 포함한다. 
...
::: {#prp-MA_open_set_is_complement_of_closed_set}

거리공간 $X$ 의 부분집합 $E$ 에 대해 다음은 동치이다. 

&emsp;($1$) $E$ 는 열린 집합이다.

&emsp;($2$) $X \backslash E$ 는 닫힌 집합이다.
...
$E$ 가 열린집합임을 가정하자. $p\in (X\backslash E)'$ 일 때 $p$ 의 모든 근방은 $X\backslash E$ 의 $p$ 가 아닌 점을 포함하므로 $p\not \in E$, 즉 $p\in X\backslash E$ 이다. 따라서 $(X\backslash E)'\subset X\backslash E$ 이다. 즉 $X\backslash E$ 는 닫힌 집합이다. 

$X\backslash E$ 가 닫힌 집합임을 가정하자. $q\in E$ 이면 $X\backslash E$ 는 닫힌집합이므로 $q$ 는 $X\backslash E$ 의 극한점이 아니다. 따라서 어떤 $q$ 의 근방은 $X\backslash E$ 와 교집합이 없으므로 $E$ 에 포함된다. 따라서 $E$ 는 열린집합이다. $\square$
...
::: {#prp-MA_closed_set_is_complement_of_open_set}
거리공간 $X$ 의 부분집합 $E$ 에 대해 다음은 동치이다. 

&emsp;($1$) $E$ 는 닫힌 집합이다.

&emsp;($2$) $X \backslash E$ 는 열린 집합이다.
:::
...
::: {#prp-MA_open_and_closed_of_set_operations}
...
($2$) ($1$)  @prp-MA_open_set_is_complement_of_closed_set 와 @prp-MA_closed_set_is_complement_of_open_set 에 의해 공집합과 $X$ 는 닫힌집합이다. 
...
($2$) $E=\overline{E}$ 이면 ($1$) 에 의해 $E$ 는 닫힌 집합이다. $E$ 가 닫힌집합이면 @def-MA_topologycal_terms 3. 에 의해 $E' \subset E$ 이므로 $E=\overline{E}$ 이다.
...
($1 \implies 2$) $p\in E$ 에 대해 $N_Y(p,\,\varepsilon_p)\subset E$ 인 $\varepsilon_p>0$ 이 존재한다. $G=\bigcup_{p\in E} N_X(p,\,\varepsilon_p)$ 라고 하면 $G$ 는 열린 집합이며 $E\subset Y \cap G$ 이다. 또한 각각의 $p\in E$ 에 대해 $N_X(p,\,\varepsilon_p) \cap Y \subset E$ 이므로 $Y \cap G \subset E$ 이다. 즉 $Y = Y \cap G$ 이다.
...
유리수의 조밀성(@prp-FA_Q_is_dense) 을 생각하자. $x\in \mathbb{R}$ 이라면 임의의 $N_\mathbb{R}(x,\,\delta)$ 내에 항상 유리수가 존재한다. $\square$

:::


다음은 유계이며 닫힌 실수의 부분집합의 성질로서 매우 중요하다.
<div class="border" style="background-color:#FFF0F5 ;padding:5px;">
::: {#prp-MA_bounded_and_closed_subset_of_real_number_and_maximum_and_minimum}

#### 유계이며 닫힌 실수의 부분집합

$A\subset \R$ 이 유계이고 닫힌 집합이면 $A$ 는 최소값과 최대값을 가진다.
...
$A$ 가 유계이므로 상한과 하한이 존재한다. $M =\max (A)$ 라고 하자. $M$ 이 $A$ 의 극한점이라면 $A$ 가 닫힌 집합이므로 $M\in A$ 이다. $M$ 이 극한점이 아니라고 하자. $M\not\in A$ 이면 어떤 $\delta >0$ 에 대해 $N_\R(M,\, \delta) \in \R \backslash A$ 이므로 $M$ 은 $A$ 의 상한이 아닌데 이는 가정에 모순된다. 따라서 $M\in A$ 이다. $m=\min (A)$ 에 대해서도 똑같이 보일 수 있다. 즉 $A$ 는 최소값과 최대값을 가진다. $\square$
...
거리공간 $X$ 의 부분집합 $E$ 에 대한 어떤 열린 덮개 $\{G_\alpha\}$ 에 대해서도 유한개의 $\{G_{\alpha}\}$ 의 원소만으로 $E$ 의 열린 덮개가 된다면 $E$ 를 **옹골집합(compact set)** 이라고 한다.

::: 
:::

즉 어떤 무한한 열린 덮게 $\{G_\alpha\}$ 를 잡더라도 유한개의 $G_\alpha$ 의 합집합으로 $E$ 를 포함 할 수 있다면 $E$ 는 옹골집합이다.

</br><div class="border" style="background-color:#FFF0F5 ;padding:5px;">

::: {#prp-MA_compact_relative}
...
($1 \implies 2$) $\{V_\alpha\}$ 를 $Y$ 에서의 $K$ 에 대한 열린 덮개라고 하자. @prp-MA_open_closed_relative  에 따라 각각의 $V_\alpha$ 에 대해 $V_\alpha = G_\alpha \cap Y$ 인 $X$ 에서의 열린 집합 $G_\alpha$ 가 존재하며 $\{G_\alpha\}$ 는 $K$ 에 대한 $X$ 에서의 열린 덮개이므로 유한개의 $G_{\alpha}$ 만으로 $K$ 를 포함 할 수 있다. 이 $G_\alpha$ 의 유한 부분집합을 $G_{\alpha_1},\ldots,\,G_{\alpha_n}$ 이라고 하면 $\{V_{k} = G_{\alpha_k} \cap Y: k=1,\ldots,\,n\}\subset \{V_\alpha\}$ 는 $K$ 에 대한 $Y$ 에서의 열린 덮개이므로 $K$ 는 $Y$ 에서의 옹골집합이다.

($2 \implies 1$) $\{U_\alpha\}$ 를 $X$ 에서의 $K$ 에 대한 열린 덮개라고 하자. @prp-MA_open_closed_relative 에 의해 각각의 $U_\alpha$ 에 대해 $V_\alpha = Y\cap U_\alpha$ 는 $Y$ 에서의 열린집합이며 $\{V_\alpha\}$ 는 $K$ 에 대한 $Y$ 에서의 열린 덮개이다. 유한개의 $V_\alpha$ 가 $K$ 를 덮으므로 유한개의 $V_{\alpha}$ 는 $K$ 를 덮는다. $\square$

:::

</br><div class="border" style="background-color:#FFF0F5 ;padding:5px;">

::: {#prp-MA_compact_set_is_closed}

거리공간에서의 옹골집합은 닫힌 집합이다.
...
::: {.proof}
거리공간 $X$ 에서 $K\subset X$ 가 옹골집합이라고 하자. 고정된 $p \in  X\backslash K$ 와 각각의 $q\in K$ 에 대해 반지름 $r$ 이 $\frac{1}{2}d(p,\,q)$ 보다 작은 $p$ 의 근방 $V_q$ 와 $q$ 의 근방 $W_q$ 를 잡을 수 있다. 집합족 $\{W_q\}$ 는 $K$ 에 대한 열린 덮개이므로 유한개의 근방들을 모아 $K$ 를 덮을 수 있다. 이 유한개의 근방을 각각 $W_{q_1},\ldots,\,W_{q_n}$ 이라고 하고 이 $q_1,\,\ldots,\,q_n$ 의 근방을 정할때의 $p$ 의 근방을 각각 $V_{q_1},\ldots,\,V_{q_n}$ 이라고 하자.

$W=W_{q_1} \cup \cdots \cup W_{q_n}$ 라 하고 $V=V_{q_1}\cap \cdots \cap V_{q_n}$ 라 하자. $V$ 는 $p$ 의 근방이며 $W \cap V = \varnothing$ 이다. 따라서 $V\subset X\backslash K$ 이므로 $X\backslash K$ 는 열린집합이다. 즉 $K$ 는 닫힌 집합이다. $\square$ 

:::

</br><div class="border" style="background-color:#FFF0F5 ;padding:5px;">

::: {#prp-MA_closed_subset_of_compact_set_is_compact}
...
::: {#cor-MA_intersection_of_closed_and_compact_set}
...
### 옹골집합의 기본 성질 {#sec-MA_basic_properties_of_compact_set}

<div class="border" style="background-color:#FFF0F5 ;padding:5px;">

::: {#prp-MA_finite_intesection_of_compact_sets}
...
::: {#prp-MA_infinite_subset_in_compact_set_has_limit_point_in_compact_set}
...
::: {#prp-MA_intersection_of_intervals}
...
아래 명제는 위의 명제로부터 자명하다.
<div class="border" style="background-color:#FFF0F5 ;padding:5px;">

::: {#prp-MA_intersection_of-k-cells}
...
### 하이네-보렐 정리 {#sec-MA_heine_borel_theorem}
...
($1 \implies 2$) $E$ 가 유계이므로 $E$ 를 포함하는 $n$-cell 이 존재하며 @prp-MA_closed_subset_of_compact_set_is_compact 에 의해 $E$ 는 옹골집합이다.

($2 \implies 3$) @prp-MA_infinite_subset_in_compact_set_has_limit_point_in_compact_set 에 의해 성립한다.
...
::: {#cor-MA_minimum_and_maximum_of_compact_sebset_in_R}

#### 실수의 부분집합인 옹골집합의 최대값과 최소값

$K\subset \R$ 이 옹골집합이면 $K$ 의 최소값과 최대값을 가진다.
...
::: {.proof}
@prp-MA_bounded_and_closed_subset_of_real_number_and_maximum_and_minimum $\square$
:::


</br><div class="border" style="background-color:#FFF0F5 ;padding:5px;">

::: {#thm-MA_bolzano_weierstrass}
...
### 완전집합 {#sec-MA_perfect_set}

<div class="border" style="background-color:#FFF0F5 ;padding:5px;">

::: {#thm-MA_perpect_set_is_uncountable}
...
$P$ 를 $\mathbb{R}^n$ 에서의 완전집합이라고 하자. 정의(@def-MA_topologycal_terms) 에 의해 $p\in P$ 는 $P$ 극한점이며 @prp-MA_limit_points_1 에 의해 $P$ 는 무한집합이다. $P$ 가 가산집합임을 가정하고 각각의 $P$ 를 $x_1,\,x_2,\ldots$ 라고 하자. 그리고 근방의 수열 $\{V_n\}$ 을 다음과 같이 정의한다.
...
이라고 하자. $P=\displaystyle \bigcap_{k=1}^\infty E_k$ 를 **칸토어 집합(Contor set)** 이라고 한다. 각각의 $E_k$ 는 유계인 닫힌 집합이므로 옹골집합이고 따라서 $P$ 는 옹골집합이다 $E_1 \supset E_2 \supset \cdots$ 이며 각각의 $E_k$ 는 공집합이 아니므로 @prp-MA_finite_intesection_of_compact_sets 에 의해 $P$ 는 공집합이 아니다.
...
### 연결집합 {#sec-MA_connected_set}

::: {.callout-note icon="false"}

::: {#def-MA_connected_set}
...
::: {.solution}
$x\notin E'$  이면 $N(x,\, \epsilon) \cap E = \varnothing$ 혹은 $\{x\}$ 인 $\epsilon>0$ 이 존재한다. $y\in N(x,\, \epsilon) \cap E'$ 이라면 @prp-MA_limit_points_1 에 의해 $y$ 의 모든 근방에 무수히 많은 $E$ 의 점들이 포함되므로 $y\in N(x,\,\epsilon)$ 일 수 없다. 따라서 $N(x,\, \epsilon) \subset X\backslash E'$ 이므로 $X\backslash E'$ 은 열린 집합이다. 따라서 $E'$ 은 닫힌 집합이다.
...
($1$) $x\in \overline{B_n}$ 이면 $x\in B_n$ 이거나 $x\in {B_n}'$ 이다. $x\in B_n$ 이면 $x\in \bigcup_{i=1}^n \overline{A_i}$ 임은 자명하다. $x\in {B_n}'\backslash B_n$ 이면 모든 $x$ 의 근방은 무한개의 $B_n$ 의 원소를 포함한다. <u>$x$ 가 어떤 $A_i$ 에 대해서도 $x\notin \overline{A_i}$ 이면 어떤 $x$ 의 근방은 $A_i$ 의 원소를 하나도 포함하지 않으며 따라서 이런 근방중 가장 작은 근방은 $B_n$ 의 원소를 포함하지 않는데</u> 이는 $x\in {B_n}'$ 에 모순된다. 따라서 $x$ 는 어떤 $A_i$ 에 대해 $x\in \overline{A_i}$ 이며 따라서 $\overline{B_n} \subset \bigcup_{i=1}^n \overline{A_i}$ 이다. 
...
::: {#prp-MA_closed_set_and_boundary}
...
일 때 $x$ 를 $A$ 의 **경계점** 이라고 하며 $A$ 의 경계점의 집합을 $\partial A$ 혹은 $\text{Bd}(A)$ 라고 표기한다. 
...
($1$) @prp-MA_closed_set_and_boundary 에 의해 $\partial A = \overline{A} \cap \overline{\mathbb{R}^n\backslash A}$ 이다. 닫힌 집합의 교집합으로 정의되므로 $\partial A$ 는 닫힌 집합이다.
...
$X\subset A\times B$ 가 무한집합이라 하자. @thm-MA_heine_borrel 에 의해 $X$ 가 $A\times B$ 에서의 극한점을 가지면 $A\times B$ 는 옹골집합이다. 
...
@exr-MA_rudin_2_24 에 의해 $X$ 는 separable 이다. @exr-MA_rudin_2_23 에 의해 $X$ 는 countable base 를 갖는다. 이제 $\{G_\alpha\}$ 를 $X$ 에 대한 열린 덮개라고 하자. $\{G_\alpha\}$ 가 불가산 집합족이면 가산무한개의 $\{G_\alpha\}$ 의 부분집합족으로 $X$ 를 덮을 수 있음을 보이자. $\{\Theta_i\}$ 를 $X$ 의 countable basis 라 하자. 각각의 $G_\alpha$ 는 $\{\Theta_i\}$ 의 부분 합집합으로 표현 할 수 있으며 $\{\Theta_i\}$ 의 부분합집합의 갯수는 가산무한이다(@thm-MA_countable_union_of_countable_sets). 따라서 가산무한개의 $\{G_\alpha\}$ 의 부분집합족으로 $X$ 를 덮을 수 있다. 이것을 $\{G_i:i\in\mathbb{N}\}$ 이라 하자. $\varnothing \not\in \{G_i\}$ 라고 정해도 무방하다. 이제 어떤 유한개의 $\{G_i\}$ 의 부분집합족도 $X$ 를 덮을 수 없음을 가정하자. 
...
$V\subset \mathbb{R}$ 이 불가산개의 열린 구간의 합집합이라고 하자. 즉 $V=\bigcup_{\alpha} I_{\alpha}$, $\alpha \ne \beta \implies I_{\alpha} \cap I_{\beta} = \varnothing$ 이다. @exr-MA_rudin_2_22 에서 $\mathbb{R}$ 이 separable 임을 보였다. 또한 @exr-MA_rudin_2_23 으로부터 $\mathbb{R}$ 은 가산무한개의 기저를 가짐을 안다. 따라서 각각의 $I_\alpha$ 는 이 기저의 부분 합집합이다. 그런데 가산무한개의 집합족의 부분합집합의 갯수는 가산무한개이므로 서로 다른 불가산개의 열린 구간이 존재 할 수 없다(@exr-MA_rudin_2_26). 따라서 열린 집합 $V$ 는 가산개의 열린 구간의 합집합이다. 
