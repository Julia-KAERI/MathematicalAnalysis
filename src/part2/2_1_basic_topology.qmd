...
::: {#cor-MA_closed_set_is_complement_of_open_set}
거리공간 $X$ 의 부분집합 $E$ 에 대해 다음은 동치이다. 
...
($2$) ($1$)  @prp-MA_open_set_is_complement_of_closed_set 와 @cor-MA_closed_set_is_complement_of_open_set 에 의해 공집합과 $X$ 는 닫힌집합이다. 
...
($1 \implies 2$) $p\in E$ 에 대해 $N_Y(p,\,\varepsilon_p)\subset Y$ 인 $\varepsilon_p>0$ 이 존재한다. $G=\bigcup_{p\in E} N_X(p,\,\varepsilon_p)$ 라고 하면 $G$ 는 열린 집합이며 $E\subset Y \cap G$ 이다. 또한 각각의 $p\in E$ 에 대해 $N_X(p,\,\varepsilon_p) \cap Y \subset E$ 이므로 $Y \cap G \subset E$ 이다. 즉 $Y = Y \cap G$ 이다.
...
::: {#cor-MA_intersection_of-k-cells}
