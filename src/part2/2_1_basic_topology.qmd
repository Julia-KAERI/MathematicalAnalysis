...
임의의 $n\in \mathbb{Z}_+$ 에 대해 $\left\{ N (x,1/n) : x\in K \right\}$ 는 $K$ 에 대한 열린 덮개이므로 유한개만을 선택하여 $K$ 를 덮을 수 있다. 이 유한개의 근방의 중심을 $x^n_1,\ldots,\,x^n_{m_n}$ 이라 하고 $N^n_i = N(x_i,\, 1/n)$ 이라 하자. $\{N^n_i : n=1,\,2,\ldots,\, i=1,\ldots,\, m_n\}$ 는 countable set 이며 $K$ 의 기저이고 $E=\{x^n_i : n=1,\,2,\ldots,\, i=1,\ldots,m_n,\}$ 일 때 $\overline{E}=K$ 이므로 $K$ 는 separable 이다. 
